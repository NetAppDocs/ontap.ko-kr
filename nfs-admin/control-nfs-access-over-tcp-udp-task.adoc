...
summary: -tcp 및 -udp 매개변수를 각각 수정하여 TCP 및 UDP를 통한 스토리지 가상 머신의 NFS 액세스를 활성화 또는 비활성화할 수 있습니다. 이를 통해 환경에서 NFS 클라이언트가 TCP 또는 UDP를 통해 데이터에 액세스할 수 있는지 여부를 제어할 수 있습니다. 
---

...
summary: NFS 클라이언트가 NTFS 보안 스타일 데이터에 액세스하고 NTFS 클라이언트가 NFS 보안 스타일 데이터에 액세스할 수 있도록 ONTAP를 구성할 수 있습니다. NFS 데이터 저장소에서 NTFS 보안 스타일을 사용하는 경우 루트 사용자의 액세스를 처리하는 방법을 결정하고 그에 따라 스토리지 가상 머신을 구성해야 합니다. 
---

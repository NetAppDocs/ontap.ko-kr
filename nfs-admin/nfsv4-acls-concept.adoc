...
summary: NFSv4 ACL을 사용하는 클라이언트는 시스템의 파일 및 디렉터리에 대한 ACL을 설정하고 볼 수 있습니다. ACL이 적용된 디렉터리에 새 파일이나 하위 디렉터리가 생성되면 새 파일이나 하위 디렉터리는 적절한 상속 플래그가 지정된 ACL의 모든 액세스 제어 항목을 상속받습니다. 
---

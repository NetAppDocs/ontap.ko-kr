...
ONTAP 9.9.1 이상에서는 NFSv4.2가 기본적으로 활성화되어 있습니다.  ONTAP 9.8에서는 다음을 지정하여 v4.2를 수동으로 활성화해야 합니다. `-v4.2` 옵션 및 설정 `enabled` 스토리지 가상 머신(SVM)에 NFS 서버를 생성할 때.  NFSv4.1을 활성화하면 클라이언트가 v4.2로 마운트된 동안에도 NFSv4.1 기능을 사용할 수 있습니다.

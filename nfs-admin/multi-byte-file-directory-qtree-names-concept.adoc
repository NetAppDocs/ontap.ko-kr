...
summary: 'ONTAP 9.5부터 4바이트 UTF-8 인코딩 이름 지원이 추가되어 기본 다국어 평면(Basic Multilingual Plane) 외의 유니코드 보조 문자를 포함하는 파일, 디렉터리 및 트리 이름을 생성하고 표시할 수 있게 되었습니다. 이전 버전에서는 이러한 보조 문자가 다중 프로토콜 환경에서 올바르게 표시되지 않았습니다.' 
---
= 다중 바이트 파일, 디렉토리 및 qtree 이름에 대한 ONTAP NFS 처리에 대해 알아보세요.
:allow-uri-read: 
:icons: font
:imagesdir: ../media/

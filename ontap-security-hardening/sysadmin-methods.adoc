...
----
에 대한 자세한 내용은 `security ssh show` link:https://docs.netapp.com/us-en/ontap-cli/search.html?q=security+ssh+show["ONTAP 명령 참조입니다"^]을 참조하십시오.
...
----
에 대한 자세한 내용은 `security login banner modify` link:https://docs.netapp.com/us-en/ontap-cli/security-login-banner-modify.html["ONTAP 명령 참조입니다"^]을 참조하십시오.
...
이 `security login motd modify` 명령은 MOTD(오늘의 메시지)를 업데이트합니다. 에 대한 자세한 내용은 `security login motd modify` link:https://docs.netapp.com/us-en/ontap-cli/security-login-motd-modify.html["ONTAP 명령 참조입니다"^]을 참조하십시오.
...
명령을 사용하여 `system timeout show` 현재 CLI 세션 시간 초과를 봅니다. 시간 초과 값을 설정하려면 `system timeout modify -timeout <minutes>` 명령을 사용합니다. 및 `system timeout modify` 에 대한 자세한 `system timeout show` 내용은 을 link:https://docs.netapp.com/us-en/ontap-cli/search.html?q=system+timeout["ONTAP 명령 참조입니다"^]참조하십시오.

...
---
= ONTAP System Manager 사용자를 위한 WebAuthn 다중 요소 인증에 대해 알아보십시오
:hardbreaks:
:allow-uri-read: 
:nofooter: 
:icons: font
...
[role="lead"]
ONTAP 9.16.1부터 관리자는 System Manager에 로그인하는 사용자에 대해 WebAuthn 다단계 인증(MFA)을 활성화할 수 있습니다. 이를 통해 FIDO2 키(예: YubiKey)를 두 번째 인증 형식으로 사용하여 System Manager에 로그인할 수 있습니다. 기본적으로 WebAuthn MFA는 신규 및 기존 ONTAP 사용자에 대해 비활성화되어 있습니다.

[NOTE]
====
ONTAP 9.19.1부터 관리자는 WebAuthn 전역 설정 명령에서  `-rp-domains` 매개변수를 사용해야 합니다. 이를 통해 관리자는 System Manager에서 WebAuthn(FIDO2) 다중 요소 인증 중에 허용되는 Relying Party(RP) 도메인 집합을 명시적으로 제한할 수 있습니다. ONTAP 9.16.1부터 9.18.1까지에서 ONTAP 9.19.1로 업그레이드하는 경우 자세한 내용은 link:https://kb.netapp.com/on-prem/ontap/Ontap_OS/OS-Issues/CONTAP-597688["CONTAP-597688"]을 참조하십시오.

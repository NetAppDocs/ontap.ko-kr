...
== 허용되는 RP(Relying Party) 도메인 집합을 제한합니다.

ONTAP 9.19.1 버전부터 관리자는 WebAuthn 전역 설정 명령에서 -rp-domains 매개변수를 사용해야 합니다. 이를 통해 관리자는 System Manager에서 WebAuthn(FIDO2) 다중 요소 인증 중에 허용되는 RP(Relying Party) 도메인 집합을 명시적으로 제한할 수 있습니다.

사용자는 허용된 RP 도메인에서만 MFA에 액세스할 수 있습니다.


NOTE: ONTAP 9.16.1부터 9.18.1까지의 버전에서 ONTAP 9.19.1로 업그레이드하는 경우 자세한 내용은 link:https://kb.netapp.com/on-prem/ontap/Ontap_OS/OS-Issues/CONTAP-597688["CONTAP-597688"]을 참조하십시오.

[role="tabbed-block"]
====
.시스템 관리자
--
. 클러스터 > 설정 > 다단계 인증(MFA)*을 선택합니다.
. 허용된 RP 도메인을 추가, 삭제 또는 수정합니다.


--
.CLI를 참조하십시오
--
. 허용되는 rp 도메인을 수정합니다. -rp-domains를 전체 클러스터 FQDN으로 설정하십시오.
+
[source, console]
----
...
----
+
 `security webauthn modify`에 대한 자세한 내용은 link:https://docs.netapp.com/us-en/ontap-cli/security-webauthn-modify.html#parameters["ONTAP 명령 참조입니다"^]을 참조하십시오.

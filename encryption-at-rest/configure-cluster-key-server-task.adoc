...
. 기본 키 서버를 수정하여 보조 키 서버를 추가합니다. 그만큼 `-secondary-key-servers` 매개변수는 최대 3개의 키 서버를 쉼표로 구분하여 나열할 수 있습니다. 보조 키 서버에 대한 포트 번호를 포함하지 마세요.
...
--
. 기본 키 서버를 수정하여 보조 키 서버를 추가합니다. 그만큼 `-secondary-key-servers` 매개변수는 최대 3개의 키 서버를 쉼표로 구분하여 나열할 수 있습니다. 보조 키 서버에 대한 포트 번호를 포함하지 마세요.
`security key-manager external modify-server -vserver _svm_name_ -key-servers _primary_key_server_ -secondary-key-servers _list_of_key_servers_` 보조 키 서버에 대한 자세한 내용은 다음을 참조하세요. <<mod-secondary>>.
...
보조 키 서버는 다음과 같이 관리됩니다. `-secondary-key-servers` 의 매개변수 `security key-manager external modify-server` 명령. 그만큼 `-secondary-key-servers` 매개변수는 쉼표로 구분된 목록을 허용합니다. 목록에서 보조 키 서버의 지정된 순서는 보조 키 서버의 액세스 순서를 결정합니다. 명령을 실행하여 액세스 순서를 수정할 수 있습니다. `security key-manager external modify-server` 2차 키 서버는 다른 순서로 입력됩니다. 보조 키 서버에 대한 포트 번호를 포함하지 마세요.

...
* MetroCluster IP 구성에서 통신에 HTTPS를 사용하는 ONTAP Mediator 구성을 지원합니다. HTTPS는 SSL/TLS를 사용하여 전체 세션을 암호화함으로써 보안을 강화합니다. HTTPS 프로토콜만 사용하는 ONTAP Mediator를 설치하도록 선택할 수 있습니다. 이 설치 방법을 사용하는 경우 SCST 패키지는 설치되지 않습니다. link:index.html#https-iscsi-installation["ONTAP Mediator의 HTTPS 및 iSCSI 설치 옵션에 대해 자세히 알아보십시오"].

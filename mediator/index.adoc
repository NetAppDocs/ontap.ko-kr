...
* ONTAP Mediator의 어떤 버전이든 사용하여 MetroCluster IP 구성을 모니터링할 수 있습니다.
...
ONTAP Mediator를 업그레이드하거나 설치하기 전에 다음 사항을 검토하십시오.
...
* 새로운 ONTAP Mediator 버전으로 업그레이드할 때, 더 높은 버전이 없는 한 설치 프로그램은 자동으로 권장 SCST 버전으로 업그레이드합니다. 더 높은 SCST 버전을 수동으로 설치하는 방법에 대한 지침은 link:manage-task.html["ONTAP 중재자 관리"]을 참조하십시오. 지원되는 버전은 link:whats-new-concept.html#scst-support-matrix["SCST 지원 매트릭스"]을 참조하십시오.
...
.네트워킹 요구 사항
다음 포트가 ONTAP Mediator에 사용 가능하고 사용되지 않는지 확인하십시오.
...
|===
SMBC 고객의 경우 ONTAP에서는 포트 3260을 활성화하거나 연결할 필요가 없습니다.

* 타사 방화벽을 사용하는 경우 link:https://docs.netapp.com/us-en/ontap-metrocluster/install-ip/concept_mediator_requirements.html#firewall-requirements-for-ontap-mediator["ONTAP Mediator의 방화벽 요구 사항"^]을(를) 참조하십시오.
...
* 테스트를 거친 SCST 모듈이 제대로 작동하지 않을 수 있으므로 ONTAP Mediator 릴리스에서 지원하는 OS 버전 이상으로 커널을 업그레이드하지 마십시오.

...
* ONTAP Mediator를 iSCSI 프로토콜로 구성하면 모든 버전의 ONTAP Mediator를 사용하여 MetroCluster IP 구성을 모니터링할 수 있습니다.
* ONTAP 9.19.1 및 ONTAP Mediator 1.12부터 HTTPS 통신을 사용하는 ONTAP Mediator를 구성하여 MetroCluster IP 구성을 모니터링할 수 있습니다.
...
ONTAP Mediator를 업그레이드하거나 설치하기 전에 다음 정보를 검토하십시오.


NOTE: ONTAP Mediator 1.8 및 이전 버전은 Red Hat Enterprise Linux(RHEL) FIPS 모드와 호환되지 않아 성공적으로 설치할 수 없습니다.  FIPS 모드가 활성화되어 있는지 확인하려면 다음을 사용하세요. `fips-mode-setup --check` 명령.  다음을 사용하여 FIPS 모드를 비활성화할 수 있습니다. `fips-modesetup --disable` 명령. ONTAP Mediator 1.8 또는 이전 버전을 성공적으로 설치하려면 FIPS 모드를 비활성화한 후 재부팅하세요.

* ONTAP Mediator를 최신 버전으로 업그레이드해야 합니다.  이전 버전은 모든 ONTAP 릴리스에서 여전히 작동하지만 최신 버전에는 타사 구성 요소에 대한 보안 패치가 포함되어 있습니다.
* SCST 패키지가 포함된 새로운 ONTAP Mediator 버전으로 업그레이드할 경우, 설치 프로그램은 더 높은 버전이 없는 한 권장 SCST 버전으로 자동 업그레이드합니다. 더 높은 SCST 버전을 수동으로 설치하는 방법에 대한 지침은 link:manage-task.html["ONTAP 중재자 관리"]을 참조하십시오. 지원되는 버전은 link:whats-new-concept.html#scst-support-matrix["SCST 지원 매트릭스"]을 참조하십시오.
...
=== ONTAP Mediator의 HTTPS 및 iSCSI 설치 옵션

* ONTAP Mediator에서는 SCST 패키지를 사용하여 MetroCluster IP 구성에서 통신을 위한 iSCSI를 제공합니다. SCST 패키지는 설치 중에 컴파일되는 커널 모듈입니다. ONTAP Mediator 1.12부터는 iSCSI 대신 HTTPS를 사용하여 MetroCluster IP 구성을 모니터링하는 ONTAP Mediator를 구성할 수 있습니다.
...
ONTAP Mediator는 특정 서비스와 통신하기 위해 여러 포트를 사용합니다.

* 타사 방화벽을 사용 중인 경우:
+
** HTTPS 액세스가 활성화되어 있어야 합니다.
** 포트 31784와 3260에 대한 액세스를 허용하도록 구성해야 합니다.
+
기본 Red Hat 또는 CentOS 방화벽을 사용하는 경우 Mediator 설치 중에 방화벽이 자동으로 구성됩니다.
...
다음 표는 방화벽에서 허용해야 하는 port 목록입니다.

[NOTE]
====
* iSCSI 포트는 MetroCluster IP 구성에서만 필요합니다.
* 22/tcp 포트는 정상 작동에는 필요하지 않지만 유지보수를 위해 일시적으로 활성화하고 유지보수 세션이 완료되면 비활성화할 수 있습니다.
...
ONTAP는 iSCSI 대상을 중재합니다
 a| 
( MetroCluster IP 구성에 필요) 사서함에 대한 iSCSI 데이터 연결

|===

NOTE: HTTPS를 사용하여 통신하는 SnapMirror 액티브 동기화 또는 MetroCluster IP ONTAP 미디에이터의 경우 ONTAP에서 포트 3260을 활성화하거나 연결할 필요가 없습니다.
...
+
** 호환 가능: 10.1 및 9.7 ^1^
** 권장 사항: 10.2, 10.0, 9.8, 9.6, 9.4 및 8.10


* Rocky Linux 10.2, 9.8 및 8.10
* Oracle Linux 10.1 및 9.8
...
=== OS 업그레이드 및 커널 호환성 요구 사항

ONTAP Mediator와 함께 SCST 패키지를 설치할 경우 다음과 같은 요구 사항이 적용됩니다.


NOTE: HTTPS만 지원하는 ONTAP Mediator 1.12 이상을 설치한 경우 SCST 패키지가 설치되지 않으므로 커널 호환성 요구 사항이 적용되지 않습니다.

* 커널을 제외한 모든 라이브러리 패키지를 업데이트할 수 있지만 ONTAP Mediator에서 변경 사항을 적용하려면 재부팅이 필요할 수 있습니다.  재부팅이 필요한 경우 가동 중지 시간을 예약하세요.
* OS 커널을 최신 상태로 유지해야 합니다.  커널 코어를 지원되는 버전으로 업그레이드하세요.link:whats-new-concept.html#scst-support-matrix["ONTAP 중재자 버전 매트릭스"] .  시스템을 재부팅해야 하므로, 정전에 대비한 유지 관리 기간을 계획하세요.
+
...
* ONTAP Mediator 릴리스에서 지원하는 OS 버전 이상으로 커널을 업그레이드하지 마십시오. 테스트를 거친 SCST 모듈이 작동하지 않을 수 있습니다.
...
* HTTPS만 지원하는 ONTAP Mediator 1.12 이상을 설치한 경우 SCST 패키지가 설치되지 않으므로 이 작업을 건너뛸 수 있습니다.

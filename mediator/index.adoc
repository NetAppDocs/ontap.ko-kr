...
* ONTAP Mediator에서는 SCST 패키지를 사용하여 MetroCluster IP 구성에서 iSCSI 통신을 제공합니다. SCST 패키지는 설치 중에 컴파일되는 커널 모듈입니다. ONTAP 9.19.1 및 ONTAP Mediator 1.12 버전부터는 iSCSI 대신 HTTPS를 사용하여 MetroCluster IP 구성을 모니터링하는 ONTAP Mediator를 구성할 수 있습니다.
* ONTAP 9.19.1 및 ONTAP Mediator 1.12 버전부터는 iSCSI 및 HTTPS 지원 또는 HTTPS만 지원하는 ONTAP Mediator 설치 옵션을 선택할 수 있습니다. HTTPS만 지원하는 ONTAP Mediator를 설치하는 경우 SCST 패키지는 설치되지 않습니다.

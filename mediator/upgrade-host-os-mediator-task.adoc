...
---
= 호스트 운영 체제를 업그레이드하고 ONTAP Mediator를 다시 설치하거나 업그레이드하십시오.
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
SCST가 설치된 ONTAP Mediator의 호스트 OS를 최신 버전으로 업그레이드하려면 ONTAP Mediator를 제거해야 합니다. OS 업그레이드 후 현재 버전의 ONTAP Mediator를 다시 설치하거나 최신 버전으로 업그레이드할 수 있습니다.

.이 작업에 대해
* 이 작업은 SCST가 설치된 ONTAP Mediator 버전에 적용됩니다. HTTPS만 지원하는 ONTAP Mediator 1.12 이상을 설치한 경우 SCST 패키지가 설치되지 않으므로 ONTAP Mediator에 영향을 주지 않고 호스트 OS를 업그레이드할 수 있습니다. OS 업그레이드 지침은 호스트 OS 공급업체의 설명서를 참조하십시오. 호스트 OS를 업그레이드한 후 ONTAP Mediator 버전을 업그레이드해야 하는 경우 link:install-mediator-pkg-task.html["ONTAP 중재자 설치 패키지를 설치합니다"]을(를) 참조하십시오.

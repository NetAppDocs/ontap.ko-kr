...
[CAUTION]
====
* MetroCluster IP 구성에서 ONTAP Mediator가 HTTPS 프로토콜로 구성된 경우, 단일 ONTAP Mediator 인스턴스는 하나의 MetroCluster IP 구성만 모니터링할 수 있습니다. 여러 MetroCluster IP 구성을 동시에 모니터링하는 것은 지원되지 않습니다.
...
**  `No`ONTAP Mediator를 SnapMirror 액티브 동기화에만 사용할 계획이거나, ONTAP Mediator 인스턴스당 단일 MetroCluster IP 구성을 모니터링하기 위해 HTTPS로 ONTAP Mediator를 구성할 계획인 경우 응답하십시오.
+
또는,  `--https-only` 옵션을  `./ontap-mediator-1.12.0/ontap-mediator-1.12.0 -y` 명령어에서 사용하여 HTTPS만 지정할 수 있습니다.
...
[NOTE]
====
설치 중 서명 확인을 건너뛰려면 다음 명령을 사용하십시오. `./ontap-mediator-1.12.0/ontap-mediator-1.12.0 -y --skip-code-signature-check`

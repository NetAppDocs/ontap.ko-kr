...
summary: ONTAP Mediator를 설치하거나 업그레이드하려면 설치 패키지를 받아서 호스트에서 설치 프로그램을 실행해야 합니다. 
---
...
[role="lead"]
ONTAP Mediator를 설치하거나 업그레이드하려면 설치 패키지를 받아서 호스트에서 설치 프로그램을 실행해야 합니다.
...
* ONTAP Mediator 1.12 이상 버전을 설치하거나 업그레이드하는 동안 설치 프로그램에서 다음 질문에 답변하라는 메시지가 표시됩니다.
+
...
+
** iSCSI로 구성된 ONTAP Mediator를 사용하여 하나 이상의 MetroCluster IP 구성을 모니터링할 계획이라면 답변 `Yes`해 주십시오.
** 응답 `No` ONTAP Mediator를 SnapMirror 활성 동기화에만 사용할 계획인지, 아니면 ONTAP Mediator를 HTTPS로 구성하여 하나 이상의 MetroCluster IP 구성을 모니터링할 계획인지 답변해 주십시오.
+
또는,  `--https-only` 옵션을  `./ontap-mediator-1.12.1/ontap-mediator-1.12.1` 명령어에서 사용하여 HTTPS만 지정할 수 있습니다.



* 자동화 사용 사례의 경우,  `./ontap-mediator-1.12.1/ontap-mediator-1.12.1 -y` 명령을 사용하여 설치 프로그램 프롬프트를 건너뛸 수 있습니다.
...
[NOTE]
====
설치 중 서명 확인을 건너뛰려면 다음 명령을 사용하십시오. `./ontap-mediator-1.12.1/ontap-mediator-1.12.1 --skip-code-signature-check`
...
.HTTPS 및 iSCSI를 지원하는 ONTAP Mediator 설치 예시(콘솔 출력)
[%collapsible]
====
[listing]
----
...
----
====
.HTTPS 및 iSCSI 지원을 포함한 ONTAP Mediator 업그레이드 예시(콘솔 출력)
[%collapsible]
====
[listing]
----
...
----
====
.HTTPS만 지원하는 ONTAP Mediator 설치 예시(콘솔 출력)
[%collapsible]
====
[listing]
----
...
----
====
.HTTPS만 지원하는 ONTAP Mediator 업그레이드 예(콘솔 출력)
[%collapsible]
====
[listing]
----
...
NOTE: HTTPS만 지원하는 ONTAP Mediator 1.12 이상을 설치한 경우 SCST 패키지가 설치되지 않으므로 이 작업을 건너뛸 수 있습니다.
...
NOTE: 설치 후 시스템 출력에는 README 파일과 키 위치가 제공됩니다.
...
====
. 호스트를 재부팅하고 장치의 UEFI 부팅 관리자를 사용하여 새 MOK를 승인합니다.  제공된 암호 문구가 필요합니다. `mokutil` 유용성link:index.html#install-ontap-mediator-when-uefi-secure-boot-is-enabled["UEFI 보안 부팅이 활성화된 경우 ONTAP mediator를 설치합니다"] .
...
ONTAP Mediator가 설치된 후 systemctl status가 `mediator-scst` 실패(비활성)로 표시되는 경우 다음 단계에 따라 SCST 커널 모듈에 서명하세요.


NOTE: HTTPS만 지원하는 ONTAP Mediator 1.12 이상을 설치한 경우 SCST 패키지가 설치되지 않으므로 이 작업을 건너뛸 수 있습니다.

.단계
. 빌드 프로세스 중에 공개/비공개 키 쌍이 생성됩니다. `/opt/netapp/lib/ontap_mediator/ontap_mediator/SCST_mod_keys/` 다음 명령을 사용하여 디렉토리로 이동합니다.
+
`[root@mediator-host ~]# ls /opt/netapp/lib/ontap_mediator/ontap_mediator/SCST_mod_keys/
README.module-signing  scst_module_key.der  scst_module_key.priv
[root@mediator-host ~]#`

. 다음 명령을 실행하여 UEFI 키 저장소로 공개 키를 가져오는 프로세스를 시작합니다.
+
...
. mokutil 소프트웨어는 가져오기 프로세스 중에 이 키에 사용할 임시 비밀번호를 요청합니다.
. 가져오기 프로세스가 시작되었는지 확인하십시오. `mokutil --list-new` 그리고 시스템을 재부팅하세요.  부트로더는 EFI MOK 관리자를 시작합니다.
. 화면의 메뉴를 사용하여 SCST 커널 모듈 키를 켭니다.  부팅 후 실행 `systemctl status mediator-scst` .  서비스가 시작되면 SCST 커널 모듈이 서명됩니다.

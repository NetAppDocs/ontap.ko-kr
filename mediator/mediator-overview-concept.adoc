...
ONTAP Mediator는 iSCSI를 제공하기 위해 SCST에 의존합니다( http://scst.sourceforge.net/index.html[] 참조). 이 패키지는 설치 중에 커널용으로 특별히 컴파일되는 커널 모듈입니다. 커널이 업데이트되면 SCST를 다시 설치해야 할 수 있습니다. 또는 ONTAP Mediator를 제거한 후 다시 설치하고 ONTAP 관계를 재구성하십시오.


NOTE: 서버 OS 커널에 대한 모든 업데이트는 ONTAP의 유지보수 윈도우와 조율되어야 합니다.

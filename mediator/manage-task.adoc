...
NOTE: HTTPS로 구성된 ONTAP Mediator를 사용하는 SnapMirror Business Continuity(SM-BC) 클러스터 및 MetroCluster IP 구성의 경우  `intermediate.crt` 및  `root_ca.crt` 인증서를 ONTAP 클러스터에 추가해야 합니다. link:../snapmirror-active-sync/mediator-install-task.html["SnapMirror Active Sync를 위한 ONTAP Mediator 및 클러스터 구성"] 및 link:https://docs.netapp.com/us-en/ontap-metrocluster/install-ip/task_configuring_the_ontap_mediator_service_from_a_metrocluster_ip_configuration.html["ONTAP Mediator를 MetroCluster IP 구성에 맞게 구성합니다"^]를 참조하십시오.

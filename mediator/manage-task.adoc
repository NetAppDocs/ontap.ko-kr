...
ONTAP Mediator를 실행하는 VM을 최신 커널로 업그레이드해야 하는 경우 ONTAP Mediator에서 사용하는 SCST 커널 모듈과 호환성 문제가 발생할 수 있습니다.  이 시나리오에서 NetApp SCST를 수동으로 제거했다가 다시 설치할 것을 권장합니다.
...
NOTE: 처음으로 설치를 수행하고 ONTAP Mediator를 설치하기 전에 SCST를 미리 설치하려는 경우 이 단계를 건너뛸 수 있습니다.  ONTAP Mediator 설치 프로그램은 설치 중에 SCST 구성 요소에 관련 패치를 적용합니다.

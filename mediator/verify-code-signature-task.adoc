...
[role="lead"]
NetApp에서는 ONTAP 중재자 설치 패키지를 설치하기 전에 ONTAP 중재자 코드 서명을 확인하는 것이 좋지만 이 프로세스는 선택 사항입니다.
...
[NOTE]
====
* 2025년 6월 15일부터 코드 서명 검증 인증서가 만료되어 ONTAP Mediator 1.8 및 1.9를 설치하거나 업그레이드할 수 없습니다. 대신 ONTAP Mediator 1.9.1을 설치하거나 업그레이드하세요.
...
----
.  `ontap-mediator-1.9.1.sig.tsr`연결된 인증서를 사용하여 및 `ontap-mediator-1.9.1.tsr` 파일을 확인합니다.
+
`openssl ts -verify`

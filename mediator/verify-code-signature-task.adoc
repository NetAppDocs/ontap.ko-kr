...
[NOTE]
====
* 2025년 6월 15일부터 코드 서명 검증 인증서가 만료되어 ONTAP Mediator 1.9 및 1.8를 설치하거나 업그레이드할 수 없습니다. 대신 ONTAP Mediator 1.10을 설치하거나 업그레이드하세요.
...
----
. 확인하다  `ontap-mediator-1.10.sig.tsr` 그리고  `ontap-mediator-1.10.tsr` 연관된 인증서를 사용하는 파일:
+
`openssl ts -verify`

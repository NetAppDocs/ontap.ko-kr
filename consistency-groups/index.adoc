...
ONTAP REST API에 대한 자세한 내용은 https://docs.netapp.com/us-en/ontap-automation/reference/api_reference.html#access-a-copy-of-the-ontap-rest-api-reference-documentation["ONTAP REST API 참조 설명서"^]를 참조하십시오.
...
System Manager의 일관성 그룹 메뉴에 있는 **개요** 탭에서 모니터링 데이터를 확인하거나 ONTAP REST API를 통해 요청할 수 있습니다. ONTAP 9.14.1부터는 ONTAP CLI에서  `consistency-group metrics show` 명령을 사용하여 일관성 그룹 메트릭을 확인할 수 있습니다.  `consistency-group metrics show`에 대한 자세한 내용은 link:https://docs.netapp.com/us-en/ontap-cli/vserver-consistency-group-metrics-show.html["ONTAP 명령 참조입니다"^]을 참조하십시오.


NOTE: ONTAP 9.13.1에서는 ONTAP REST API를 통해서만 과거 메트릭을 검색할 수 있습니다. ONTAP 9.14.1부터는 System Manager에서도 과거 메트릭을 사용할 수 있습니다.



== 일관성 그룹 보호

일관성 그룹은 애플리케이션 정합성 보장 보호 기능을 제공하여 여러 볼륨 또는 LIF에서 데이터의 일관성을 보장합니다. 일관성 그룹의 스냅샷을 생성할 때 일관성 그룹에 "펜스"가 설정됩니다. 이 펜스는 스냅샷 작업이 완료될 때까지 I/O 대기열을 시작하여 일관성 그룹의 모든 엔터티에서 데이터의 특정 시점 일관성을 보장합니다. 예약된 스냅샷 정책이나 System Manager를 사용한 스냅샷 생성과 같은 스냅샷 생성 작업 중에 펜스로 인해 일시적인 지연 시간이 발생할 수 있습니다. ONTAP REST API 및 ONTAP CLI의 컨텍스트에 대한 자세한 내용은 link:https://kb.netapp.com/on-prem/ontap/DM/REST-API["ONTAP REST API 설명서"^] 및 link:https://docs.netapp.com/us-en/ontap-cli/["ONTAP 명령 참조입니다"^]을 참조하십시오.
...
미러링된 애그리게이트에 배치된 일관성 그룹과 연결된 볼륨 및 스냅샷은 원격 사이트(사이트 B)로 복제됩니다. 사이트 B의 볼륨 내용은 일관성 그룹에 대한 쓰기 순서 보장을 제공하므로, 재해 발생 시 사이트 B에서 복구할 수 있습니다. ONTAP 9.11.1 이상 버전이 실행되는 클러스터에서는 ONTAP REST API 및 System Manager를 사용하여 일관성 그룹 스냅샷에 액세스할 수 있습니다. ONTAP 9.14.1부터는 ONTAP CLI를 사용해서도 스냅샷에 액세스할 수 있습니다.
...
ONTAP REST API에서 생성된 일관성 그룹 스냅샷은 System Manager의 일관성 그룹 인터페이스와 일관성 그룹 ONTAP REST API 엔드포인트를 통해 관리할 수 있습니다. ONTAP 9.14.1부터는 ONTAP CLI를 사용하여 일관성 그룹 스냅샷을 관리할 수도 있습니다.

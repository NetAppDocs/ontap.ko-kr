...
* 스냅샷 생성을 호출할 때 5초에서 120초 사이의 선택적 타임아웃 값을 설정할 수 있습니다. 타임아웃 값을 지정하지 않으면 기본값인 7초 후에 작업이 종료됩니다. ONTAP REST API에서는 `action_timeout` 매개변수를 사용하여 타임아웃 값을 설정합니다. ONTAP CLI에서는 `-timeout` 플래그를 사용합니다.


.단계
ONTAP REST API 또는 ONTAP 9.14.1 버전부터는 ONTAP CLI를 사용하여 2단계 스냅샷을 완료할 수 있습니다. 이 작업은 System Manager에서는 지원되지 않습니다.


NOTE: ONTAP REST API를 사용하여 스냅샷을 생성하는 경우, ONTAP REST API를 사용하여 스냅샷을 커밋해야 합니다. ONTAP CLI를 사용하여 스냅샷을 생성하는 경우, ONTAP CLI를 사용하여 스냅샷을 커밋해야 합니다. 두 가지 방법을 혼합하여 사용하는 것은 지원되지 않습니다.

...
summary: Hyper-V 및 SQL Server over SMB 구성을 사용하여 스토리지 가상 머신을 구성하고 클러스터의 각 노드에 LIF를 설정하더라도 일상적인 운영 중에 일부 LIF가 다른 노드의 포트로 이동할 수 있습니다. LIF 상태를 확인하고 필요한 수정 조치를 취해야 합니다. 
---
= LIF 상태를 확인합니다
:allow-uri-read: 
:icons: font

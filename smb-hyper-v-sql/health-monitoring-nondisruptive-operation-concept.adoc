...
summary: 상태 모니터링은 클러스터 전체의 시스템 상태에 대한 정보를 제공합니다. 상태 모니터는 Hyper-V 및 SMB를 통한 SQL Server 구성을 모니터링하여 애플리케이션 서버의 무중단 운영을 보장합니다. 상태가 저하된 경우 가능한 원인 및 권장 복구 조치를 포함한 문제에 대한 세부 정보를 확인할 수 있습니다. 
---

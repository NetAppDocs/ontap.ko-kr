...
summary: 지속적으로 사용 가능한 SMB 3.0 이상 파일 공유를 사용하여 Hyper-V 가상 머신 파일이나 SQL Server 시스템 데이터베이스 및 사용자 데이터베이스를 SVM에 있는 볼륨에 저장할 수 있으며, 동시에 계획된 이벤트와 계획되지 않은 이벤트 모두에 대해 무중단 운영을 제공할 수 있습니다. 
---
...
* NetApp Backup and Recovery는 CIFS 공유에 호스팅된 독립형 Hyper-V 가상 머신, FCI 클러스터 인스턴스 및 System Center Virtual Machine Manager(SCVMM)로 프로비저닝된 가상 머신을 보호합니다.
+
NetApp Backup and Recovery를 사용하여 Hyper-V 워크로드를 보호하는 방법에 대한 자세한 내용은 https://docs.netapp.com/us-en/data-services-backup-recovery/br-use-hyperv-protect-overview.html["Hyper-V 워크로드 보호 개요"]을 참조하십시오.

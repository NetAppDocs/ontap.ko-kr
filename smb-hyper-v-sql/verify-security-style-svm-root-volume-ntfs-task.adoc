...
summary: Hyper-V 및 SMB를 통한 SQL Server의 무중단 운영을 보장하려면 볼륨을 NTFS 보안 스타일로 생성해야 합니다. 스토리지 가상 머신에 생성된 볼륨에는 기본적으로 루트 볼륨의 보안 스타일이 적용되므로 루트 볼륨의 보안 스타일을 NTFS로 설정해야 합니다. 
---

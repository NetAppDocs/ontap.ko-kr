...
summary: pNFS는 nconnect, 세션 트렁킹, 고급 용량 균형 조정 등 다양한 ONTAP 기능과 함께 사용하여 성능과 유연성을 향상시킬 수 있습니다. 
---
...
[role="lead"]
pNFS를 다양한 ONTAP 기능과 함께 사용하여 성능을 향상시키고 NFS 워크로드에 추가적인 유연성을 제공할 수 있습니다.
...
ONTAP에서 NFSv4.1 세션 트렁킹은 클러스터 내 단일 노드의 여러 물리적 인터페이스에 걸쳐 사용되어 독립적인 연결 전반에 걸쳐 추가적인 처리량과 이중화를 제공합니다.

image::pNFS_fig_006.png[단일 노드의 여러 인터페이스에 걸친 NFSv4.1 세션 트렁킹]

pNFS와 함께 사용하면 이러한 트렁크는 전체 클러스터에 걸쳐 더 많은 전반적인 집계 성능을 제공할 수 있습니다.

image::pNFS_fig_021.png[클러스터 전체에서 pNFS 성능을 지원하는 NFSv4.1]

세션 트렁킹은 여러 가지 방법으로 설정됩니다.

* *마운트 옵션을 통한 자동 검색:* 대부분의 최신 NFS 클라이언트에서 세션 트렁킹은 마운트 옵션(운영 체제 공급업체의 설명서 참조)을 사용하여 설정되며, 이 옵션은 NFS 서버에 세션 트렁크에 대한 정보를 클라이언트로 다시 전송하도록 신호를 보냅니다. 이 정보는 NFS 패킷을 통해 `fs_location4` 호출로 나타납니다.
+
...
image::pNFS_fig_017.png[ONTAP 에서 pNFS를 사용한 NFSv4.1 세션 트렁킹]
 ...
 `trunkdiscovery`을 사용하면 마운트 인터페이스가 위치한 NFS 서버 노드의 나열된 세션 트렁크 인터페이스에 대해 추가 GETATTR 호출(FS_Locations)이 활용됩니다. 해당 주소가 반환되면 후속 마운트는 반환된 주소로 수행됩니다. 이는 마운트 중 패킷 캡처에서 확인할 수 있습니다.
...
pNFS에서 NFSv4.1 리퍼럴을 사용할 수 있지만 이 기능은 필요하지 않습니다. pNFS에서 리퍼럴을 활성화해도 눈에 띄는 결과가 표시되지 않습니다.

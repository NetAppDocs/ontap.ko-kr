...
summary: 'ONTAP 9부터 iSCSI LIF는 호스트 운영 체제가 iSCSI SendTargets 검색 작업을 보낼 때 정규화된 도메인 이름(FQDN)을 반환하도록 구성할 수 있습니다. FQDN을 반환하는 것은 호스트 운영 체제와 스토리지 서비스 사이에 네트워크 주소 변환(NAT) 장치가 있는 경우에 유용합니다.' 
---

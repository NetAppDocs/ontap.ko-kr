...
summary: 'Internet Storage Name Service는 TCP/IP 스토리지 네트워크에서 iSCSI 장치의 자동 검색 및 관리를 지원하는 프로토콜입니다. iSNS 서버는 네트워크에서 활성 iSCSI 장치에 대한 정보(IP 주소, iSCSI 노드 이름 IQN, 포털 그룹 등)를 유지 관리합니다.' 
---

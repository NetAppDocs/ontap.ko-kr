...
* LUN의 특정 데이터 하위 집합(볼륨 그룹 내의 특정 논리 볼륨 또는 파일 시스템, 또는 파일 시스템 내의 특정 파일 또는 파일 집합)에 액세스하여 원래 LUN에 복사하되, 원래 LUN의 나머지 데이터는 복원하지 않으려는 경우 이 기능을 사용할 수 있습니다. 이 기능은 LUN과 LUN 복제본을 동시에 마운트할 수 있는 운영 체제에서 작동합니다.

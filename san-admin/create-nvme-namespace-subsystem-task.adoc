...
[role="lead"]
스토리지를 프로비저닝하면 NVMe 호스트가 ONTAP 스토리지 시스템에서 데이터를 읽고 쓸 수 있게 됩니다. NVMe 프로토콜을 사용하는 시스템의 경우 하나 이상의 NVMe 네임스페이스와 서브시스템을 생성해야 합니다. 각 네임스페이스는 호스트에서 데이터 액세스를 허용하도록 NVMe 서브시스템에 매핑할 수 있습니다.

NVMe 서브시스템을 포함하는 스토리지 가상 머신(SVM)에 NVMe/FC 및 NVMe/TCP 네트워크 인터페이스가 모두 있는 경우, NVMe 서브시스템에 매핑된 네임스페이스는 NVMe/FC 및 NVMe/TCP 전송 프로토콜을 통해 모두 액세스할 수 있습니다. FC에 대한 NVMe 지원은 ONTAP 9.4 이상에서 사용할 수 있습니다. TCP에 대한 NVMe 지원은 ONTAP 9.10.1 이상에서 사용할 수 있습니다. 네임스페이스에 대한 프로토콜별 액세스가 필요한 경우 각 프로토콜에 대해 link:configure-svm-nvme-task.html["SVM 구성"]해야 합니다.

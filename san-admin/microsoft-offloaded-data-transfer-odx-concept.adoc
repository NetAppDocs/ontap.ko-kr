...
summary: 'Microsoft Offloaded Data Transfer(복사 오프로드라고도 함)는 호스트 컴퓨터를 거치지 않고 스토리지 장치 내에서 또는 호환되는 스토리지 장치 간에 데이터를 직접 전송할 수 있도록 합니다.' 
---
...
가상화된 환경에서 VMware와 Microsoft 복사 오프로드를 사용하는 경우 LUN을 정렬해야 합니다.  정렬되지 않은 LUN은 성능을 저하시킬 수 있습니다. link:io-misalignments-properly-aligned-luns-concept.html["정렬되지 않은 LUN에 대해 자세히 알아보세요"].

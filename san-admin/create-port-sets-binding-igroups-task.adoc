...
summary: 선택적 LUN 맵을 사용하는 것 외에도 포트셋을 생성하고 해당 포트셋을 igroup에 바인딩하여 이니시에이터가 LUN에 액세스하는 데 사용할 수 있는 LIF를 더욱 제한할 수 있습니다. 포트셋을 igroup에 바인딩하지 않으면 igroup의 모든 이니시에이터는 LUN을 소유한 노드와 해당 노드의 HA 파트너에 있는 모든 LIF를 통해 매핑된 LUN에 액세스할 수 있습니다. 
---
= 포트셋 및 igroup에 바인딩
:allow-uri-read: 
:icons: font

...
summary: FC 이니시에이터 및 FC 타겟 모드를 지원하려면 UTA2 포트를 Converged Network Adapter 모드에서 Fibre Channel 모드로 변경해야 합니다. 포트를 네트워크에 연결하는 물리적 매체를 변경해야 하는 경우 특성을 CNA 모드에서 FC 모드로 변경해야 합니다. 
---

...
summary: ONTAP는 ESX 호스트가 ESX 4.1 이상을 실행하는 경우 특정 VMware vStorage API의 어레이 통합 기능을 지원합니다. 이러한 기능은 ESX 호스트에서 스토리지 시스템으로 작업을 오프로드하고 네트워크 처리량을 향상시킵니다. ESX 호스트는 적절한 환경에서 이러한 기능을 자동으로 활성화합니다. 
---

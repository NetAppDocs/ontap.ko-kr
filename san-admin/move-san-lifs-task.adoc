...
summary: 노드를 오프라인으로 전환해야 하는 경우, SAN LIF를 이동하여 WWPN과 같은 구성 정보를 보존하고 스위치 패브릭의 영역 변경을 방지할 수 있습니다. SAN LIF는 이동하기 전에 오프라인으로 전환해야 하므로, 호스트 트래픽은 LUN에 대한 중단 없는 액세스를 제공하기 위해 호스트 멀티패싱 소프트웨어에 의존해야 합니다. SAN LIF는 클러스터 내의 모든 노드로 이동할 수 있지만, 스토리지 가상 머신 간에는 이동할 수 없습니다. 
---
= SAN LIF를 이동합니다
:allow-uri-read: 
:icons: font

...
summary: ONTAP는 씩 프로비저닝, 씬 프로비저닝 및 세미 씩 프로비저닝의 세 가지 기본 볼륨 프로비저닝 옵션을 제공합니다. 각 옵션은 볼륨 공간과 ONTAP 블록 공유 기술의 공간 요구사항을 관리하는 방식이 서로 다릅니다. 이러한 옵션의 작동 방식을 이해하면 사용 환경에 가장 적합한 옵션을 선택할 수 있습니다. 
---
= SAN 볼륨 개요 정보
:allow-uri-read: 
:icons: font

...
summary: ONTAP는 올바르게 정렬된 LUN에서도 입력 및 출력 정렬 오류를 보고할 수 있습니다. 일반적으로 LUN이 올바르게 프로비저닝되었고 파티셔닝 테이블이 정확하다고 확신하는 경우 이러한 정렬 오류 경고는 무시해도 됩니다. 
---
...
추가 조사가 필요한 경우 다음을 참조하세요.link:https://kb.netapp.com/Advice_and_Troubleshooting/Data_Storage_Software/ONTAP_OS/How_to_identify_unaligned_IO_on_LUNs["NetApp 지식 기반: LUN에서 정렬되지 않은 IO를 식별하는 방법은 무엇입니까?"^]
...
ONTAP 9.7 또는 이전 버전에서 프로비저닝된 LUN에 대한 자세한 내용은 다음을 참조하세요.link:https://kb.netapp.com/onprem/ontap/da/SAN/How_to_identify_unaligned_IO_on_LUNs["NetApp 기술 자료: LUN에서 정렬되지 않은 IO를 식별하는 방법"^] .

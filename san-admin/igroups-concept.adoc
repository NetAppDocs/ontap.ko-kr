...
summary: 이니시에이터 그룹은 FC 프로토콜 호스트의 WWPN 또는 iSCSI 호스트 노드 이름으로 구성된 테이블입니다. 이니시에이터 그룹을 정의하고 이를 LUN에 매핑하여 어떤 이니시에이터가 LUN에 액세스할 수 있는지 제어할 수 있습니다. 
---
= ONTAP SAN igroup에 대해 알아보십시오
:allow-uri-read: 
:icons: font
:imagesdir: ../media/

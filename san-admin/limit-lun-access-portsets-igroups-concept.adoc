...
summary: 선택적 LUN 맵을 사용하는 것 외에도 igroup 및 포트셋을 통해 LUN에 대한 액세스를 제한할 수 있습니다. 
---
...
다음 예에서 host1에는 포트셋이 없습니다. 포트셋이 없으면 host1은 LIF1과 LIF2를 통해 LUN1에 액세스할 수 있습니다.

image:bsag-c-mode-no-portset.png["포트셋 없이 LUN 액세스를 보여 주는 이미지입니다"]

포트셋을 사용하여 LUN1에 대한 액세스를 제한할 수 있습니다. 다음 예에서 host1은 LIF1을 통해서만 LUN1에 액세스할 수 있습니다. 하지만 LIF2가 portset1에 없기 때문에 host1은 LIF2를 통해 LUN1에 액세스할 수 없습니다.

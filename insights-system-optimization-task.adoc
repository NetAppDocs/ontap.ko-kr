...
+
** * Dismiss *: 뷰에서 통찰력을 제거합니다. 에게  `undismiss` 통찰력을 참조하다 <<customize-settings-insights>> .
...
. 설정 * 페이지에서 알림을 받을 인사이트 옆에 있는 확인란이 있는지 확인합니다. 이전에 통찰력을 기각한 경우 다음을 수행할 수 있습니다.  `undismiss` 체크박스에 체크 표시가 되어 있는지 확인하세요.
. 저장 * 을 클릭합니다.

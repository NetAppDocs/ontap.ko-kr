...
* ARP/AI 분석은 SnapMirror 동기식 및 SnapMirror 활성 동기식 SAN 관계에서 활성 읽기/쓰기 볼륨에서 실행됩니다. ARP는 기본 볼륨에 스냅샷을 생성하고 머신 러닝 기반 랜섬웨어 탐지를 수행합니다.
* SnapMirror 동기식 또는 SnapMirror 활성 동기화 관계에서 ARP 스냅샷은 보조 볼륨으로 복제되지 않습니다. 장애 조치 후 복구 볼륨에서 link:enable-task.html["ARP를 다시 활성화합니다"]해야 합니다.

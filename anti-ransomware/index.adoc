...
[role="lead"]
ONTAP 9.10.1부터 ONTAP 관리자는 Autonomous Ransomware Protection(ARP)을 활성화하여 NAS(NFS 및 SMB) 환경에서 워크로드 분석을 수행하고 랜섬웨어 공격을 나타낼 수 있는 비정상적인 활동을 사전에 감지하고 경고할 수 있습니다. ONTAP 9.17.1부터 ARP는 LUN 또는 NVMe 네임스페이스를 포함하는 SAN 볼륨이나 VMware와 같은 하이퍼바이저의 가상 디스크를 포함하는 NAS 볼륨과 같은 블록 장치 볼륨도 지원합니다. ONTAP 9.17.1P5부터는 Hyper-V, KVM 및 OpenStack 하이퍼바이저도 지원되며, ARP는 NAS 볼륨에 포함된 가상 디스크에 대한 평가 모드를 추가합니다(<<NAS 환경 및 모드 유형>> 참조). ARP는 ONTAP에 직접 내장되어 ONTAP의 다른 기능과 통합된 제어 및 조정을 보장합니다. ARP는 실시간으로 작동하여 파일 시스템에 쓰거나 읽는 데이터를 처리하고 잠재적인 랜섬웨어 공격을 신속하게 감지하고 대응합니다.

ARP는 추가적인 보호를 위해 정기적인 간격과 예약된 간격으로 잠금 스냅샷을 생성합니다. 스냅샷 보존 기간을 스마트하게 관리합니다. 특이한 활동이 감지되지 않으면 스냅샷은 빠르게 재사용됩니다. 하지만 공격이 감지되면 공격 시작 전에 생성된 스냅샷이 장기간 보관됩니다. ONTAP 버전에 따른 변경 사항을 포함한 자세한 내용은 다음을 참조하십시오. <<ARP 스냅샷>>.

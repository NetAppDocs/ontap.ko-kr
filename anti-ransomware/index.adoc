...
[role="lead"]
ONTAP 9.10.1부터 ONTAP 관리자는 자율형 랜섬웨어 방지(ARP) 기능을 활성화하여 NAS(NFS 및 SMB) 환경에서 워크로드 분석을 수행하고 랜섬웨어 공격으로 의심되는 비정상적인 활동을 사전에 감지하고 경고할 수 있습니다. ONTAP 9.17.1부터 ARP는 LUN 또는 NVMe 네임스페이스를 포함하는 SAN 볼륨이나 VMware와 같은 하이퍼바이저의 가상 디스크를 포함하는 NAS 볼륨과 같은 블록 장치 볼륨도 지원합니다. ONTAP 9.17.1P5부터는 Hyper-V, KVM 및 OpenStack 하이퍼바이저도 지원됩니다.

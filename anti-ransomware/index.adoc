...
.새 볼륨에서 자동 기본 활성화
ONTAP 9.18.1부터 AFF A 시리즈 및 AFF C 시리즈, ASA 및 ASA r2 시스템의 모든 새 볼륨에서 ARP가 기본적으로 자동으로 활성화됩니다. 이 자동 기본 ARP 활성화는 link:use-cases-restrictions-concept.html#unsupported-configurations["지원되지 않는 볼륨 또는 구성"]에는 적용되지 않습니다.

새 볼륨에 대한 ARP 기본 활성화는 업그레이드 후 12시간의 유예 기간이 지난 후에 적용되거나, ONTAP 9.18.1을 새로 설치하는 경우에는 ARP 라이센스가 설치되어 있으면 즉시 적용됩니다. 기존 볼륨에는 <<manual-default-enablement,ARP를 수동으로 활성화합니다>>해야 합니다.

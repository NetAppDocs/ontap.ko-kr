...
[role="lead"]
ONTAP 9.10.1부터 ONTAP 관리자는 Autonomous Ransomware Protection(ARP)을 활성화하여 NAS(NFS 및 SMB) 환경에서 워크로드 분석을 수행하고 랜섬웨어 공격을 나타낼 수 있는 비정상적인 활동을 사전에 감지하고 경고할 수 있습니다. ARP는 ONTAP에 직접 내장되어 ONTAP의 다른 기능과 통합된 제어 및 조정을 보장합니다. ARP는 실시간으로 작동하여 파일 시스템에 쓰거나 읽는 데이터를 처리하고 잠재적인 랜섬웨어 공격을 신속하게 감지하고 대응합니다.

ARP는 예약된 스냅샷 외에도 정기적으로 잠금 스냅샷을 생성합니다. 스냅샷 보존 기간은 동적으로 조정됩니다. 비정상적인 활동이 감지되지 않으면 스냅샷은 빠르게 삭제되고, 공격이 감지되기 전에 생성된 스냅샷은 조사 및 복구를 위해 더 오래 보존됩니다.

프로토콜 지원, 워크로드 적합성 및 버전별 요구 사항에 대해서는 link:use-cases-restrictions-concept.html["ARP 사용 사례 및 고려 사항"]을 참조하십시오.

ONTAP 버전에 추가된 변경 사항을 포함하여 ARP에서 생성된 스냅샷에 대한 자세한 내용은 <<ARP 스냅샷>>을 참조하십시오.
...
ARP를 사용하려면 라이센스가 필요합니다. 새 볼륨에서 ARP를 기본적으로 활성화할지, 아니면 볼륨별로 수동으로 활성화할지 결정하십시오.
...
ARP 지원이 포함되어 있습니다.link:https://kb.netapp.com/onprem/ontap/os/ONTAP_9.10.1_and_later_licensing_overview["ONTAP One 라이센스"^] . ONTAP One 라이선스가 없는 경우 ONTAP 버전에 따라 ARP 사용을 위한 다른 라이선스를 사용할 수 있습니다
...
ARP는 클러스터, SVM 및 볼륨 수준에서 유연한 활성화 옵션을 제공하므로 새 볼륨에 대해 자동 기본 활성화를 구성하거나 필요에 따라 기존 볼륨에서 ARP를 수동으로 활성화할 수 있습니다.

.새 볼륨에서 자동 기본 활성화
ONTAP 9.18.1부터 AFF A 시리즈 및 AFF C 시리즈, ASA 및 ASA r2 시스템의 모든 새 볼륨에서 ARP가 기본적으로 자동으로 활성화됩니다. 이 자동 기본 ARP 활성화는 link:use-cases-restrictions-concept.html#unsupported-configurations["지원되지 않는 볼륨 또는 구성"]에는 적용되지 않습니다. 기존 볼륨에서는 <<manual-default-enablement,ARP를 수동으로 활성화합니다>>해야 합니다. 업그레이드 시 기본 활성화가 적용되는 방법에 대한 자세한 내용은 link:use-cases-restrictions-concept.html#arp-upgrade-considerations["ARP 버전별 업그레이드 고려 사항"]를 참조하십시오.


NOTE: ONTAP 9.19.1부터 SnapMirror 동기식 및 SnapMirror 액티브 동기식 SAN 관계를 포함하는 클러스터에 대해 기본적으로 ARP가 활성화됩니다. ONTAP 9.18.1에서는 이러한 구성에 대해 기본 활성화가 비활성화되었습니다. 클러스터 간 SnapMirror 동기식 관계의 경우, 소스 및 대상 클러스터 모두 ONTAP 9.19.1로 업그레이드되면 기본 활성화가 적용됩니다. AFX 시스템에서는 SnapMirror 동기식 지원 여부와 관계없이 ARP가 기본적으로 활성화되지 않습니다.

유예 기간 동안 link:default-enable-change.html["시스템 관리자 또는 ONTAP CLI를 사용하여 클러스터 수준에서 새 볼륨에 대한 기본 활성화를 해제할 수 있습니다."]. 옵트아웃하지 않으면 유예 기간이 끝난 후 생성된 모든 새 볼륨에 대해 ARP가 자동으로 활성화됩니다. 유예 기간 후에 필요 사항이 변경되면 언제든지 기본 활성화 기능을 켜거나 끌 수 있습니다.

.새 볼륨에서 수동 기본 활성화
클러스터 수준에서 ARP의 자동 기본 활성화를 비활성화한 경우 SVM 수준에서 link:enable-default-task.html["모든 새 볼륨에서 기본적으로 ARP를 수동으로 활성화합니다"]를 선택할 수도 있습니다. ONTAP 9.17.1 이하 버전에서는 새 볼륨에서 ARP를 기본적으로 활성화하도록 구성하는 유일한 방법입니다.

[[manual-default-enablement]]
.기존 볼륨 전체 또는 특정 볼륨에 대한 ARP 활성화
9.18.1부터 클러스터 레벨에서 기존의 모든 볼륨에 대해 ARP를 수동으로 활성화할 수 있습니다(*Cluster > Security*를 선택하고 *Anti-ransomware* 섹션에서 image:icon_kabob.gif["메뉴 옵션 아이콘"]를 선택한 다음 *Enable on all existing volumes*를 선택).

특정 볼륨에 대해서만 ARP 활성화를 제한하려면 link:enable-task.html["볼륨별로 ARP를 활성화합니다"]할 수 있습니다.
...
ONTAP에는 랜섬웨어로부터 보호하기 위한 FPolicy, 스냅샷, SnapLock 및 Active IQ Digital Advisor(Digital Advisor라고도 함)와 같은 기능이 포함되어 있지만, ARP는 추가적인 방어 계층을 제공합니다.
...
* link:https://docs.netapp.com/us-en/netapp-solutions-dataops/cyber-vault/ontap-cyber-vault-ps-hardening.html#ontap-cli-or-via-automation-script["PowerShell을 사용한 ONTAP 사이버 볼트 강화"^]
...
이전 ARP 모델이 적용된 버전에서는 활성 모니터링을 시작하기 전에 학습 기간을 두는 것이 좋습니다.  지원하는 NAS 환경의 경우<<arp-ai,ARP/AI>> 학습 기간이 없으며, 적극적인 모니터링이 즉시 시작됩니다.
...
| 학습  a| 
ONTAP 의 특정 버전과 특정 볼륨 유형의 경우 ARP를 활성화하면 ARP가 자동으로 학습 모드로 설정됩니다. 학습 모드에서 ONTAP 시스템은 엔트로피, 파일 확장자 유형, 파일 IOPS 등의 분석 영역을 기반으로 경고 프로필을 개발합니다.
...
FlexVol 볼륨의 ONTAP 9.16.1부터는 활성 모드만 존재하며, 이 버전 이상으로 업그레이드된 모든 FlexVol 볼륨의 경우 학습 모드가 자동으로 활성 모드로 전환됩니다.

ONTAP 9.16.1~9.17.1의 경우 FlexGroup 볼륨은 아직 ARP/AI에서 지원되지 않으며 이전 ARP 모델을 계속 실행합니다.  이러한 이유로 FlexGroup 볼륨이 포함된 이러한 버전에는 여전히 학습 기간이 권장됩니다.

ONTAP 9.18.1부터 FlexVol 및 FlexGroup 볼륨 모두에 대해 활성 모드만 존재합니다.  업그레이드된 볼륨은 자동으로 활성 모드로 전환됩니다.
...
* ONTAP 9.13.1에서 ONTAP 9.17.1로의 FlexGroup 볼륨
...
평가
 a| 
암호화 동작의 기준선을 파악하기 위해 2~4주간의 평가 기간이 진행됩니다. 기준 임계값이 설정되는 동안 탐지 및 경고가 발생할 수 있습니다. 평가 기간이 완료되었는지 여부는 `security anti-ransomware volume show`명령어를 실행하고 `Block device detection status`를 확인하여 알 수 있습니다.

link:respond-san-entropy-eval-period.html["블록 디바이스 워크로드에 대한 ONTAP ARP/AI 평가 기간에 대해 알아보십시오."].
 a| 
ONTAP 9.17.1P5 이상에서 ONTAP가 감지한 하이퍼바이저 가상 디스크가 포함된 NAS 볼륨
...
ARP는 학습된 분석 데이터를 기반으로 수신되는 데이터를 측정하여 위협 발생 확률을 평가합니다. ARP는 이상 징후를 감지하면 측정값을 할당합니다. ARP는 감지 시점 또는 정기적인 간격으로 스냅샷을 할당할 수 있습니다.
...
.버전별 ARP 스냅샷 차이점
* *보존 설정 수정*: ONTAP 9.11.1부터 link:modify-automatic-snapshot-options-task.html["스냅샷에 대한 데이터 보존 설정을 수정합니다"]할 수 있습니다.
* *간격 및 이상 감지 대응*: ONTAP 9.17.1부터 ARP 스냅샷이 NAS 및 SAN 볼륨 모두에 대해 정기적으로 생성되며, 감지된 이상 징후에 대한 응답으로도 생성됩니다. 이는 항상 표준 응답이었습니다. ONTAP는 ARP 스냅샷에 이름을 앞에 추가하여 쉽게 식별할 수 있도록 합니다.
* * SnapMirror 동기 및 활성 동기화 *: ONTAP 9.19.1부터 SnapMirror 동기 또는 SnapMirror 활성 동기화 관계에 참여하는 볼륨에 대해 생성된 ARP 스냅샷은 다음 추가 규칙을 따릅니다.
+
** ARP 스냅샷은 운영 볼륨에 생성되어 보관됩니다.
** ARP 스냅샷은 SnapMirror 동기식 또는 SnapMirror 활성 동기화의 일부로 보조 서버로 복제되지 않습니다. 장애 조치 후 복구 볼륨에서 link:enable-task.html["ARP를 다시 활성화합니다"]해야 합니다.





NOTE: 볼륨 수준 복원 (`volume snapshot restore`)을 SnapMirror 동기식 또는 SnapMirror 액티브 싱크 SAN 볼륨에서 수행하려면 SnapMirror 동기식 또는 SnapMirror 액티브 싱크 관계를 일시적으로 정지하거나 분리해야 할 수 있습니다. 많은 경우, FlexClone 또는 ARP나 기본 볼륨의 다른 스냅샷에서 파일 수준 복원 작업을 사용하여 SnapMirror 동기식 또는 SnapMirror 액티브 싱크를 분리하지 않고도 복원이 가능합니다.

link:use-cases-restrictions-concept.html#snapmirror["SnapMirror 동기식 및 SnapMirror 활성 동기화 지원에 대해 자세히 알아보십시오"].
...
.ARP/AI에 대한 능동적 보호로의 즉각적인 전환
ARP/AI를 사용하면<<ARP 모드에 대해 알아보세요,학습 기간>> .  ARP/AI는 다음 지원되는 볼륨 유형에 대해 설치 또는 업그레이드 직후에 활성화됩니다.

* ONTAP 9.16.1 이상을 사용한 NAS FlexVol 볼륨
* ONTAP 9.18.1 이상을 사용한 NAS FlexGroup 볼륨
* ONTAP 9.17.1 이상을 사용하는 SAN 볼륨(즉시 활성화됨,link:respond-san-entropy-eval-period.html["평가 기간"] )


ARP 기능이 이미 활성화된 기존 볼륨과 새 볼륨의 경우, 클러스터를 ARP/AI 지원 ONTAP 버전으로 업그레이드하면 ARP/AI 보호가 자동으로 활성화됩니다.
...
| ONTAP 버전 | ONTAP 9.10.1-9.15.1 | ONTAP 9.16.1 이상, 9.15.1(기술 미리 보기) 
...
| 평가 기간 | 해당 없음 | 기준 암호화 임계값을 설정하기 위해 SAN 볼륨에 필요(2~4주) 
|===
.관련 정보

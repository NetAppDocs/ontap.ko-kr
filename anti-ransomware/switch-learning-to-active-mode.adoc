...
[role="lead"]
NAS 환경의 경우, ARP 지원 볼륨을 학습 모드에서 활성 모드로 수동 또는 자동으로 전환할 수 있습니다. ONTAP 9.15.1 및 이전 버전에서 ARP를 사용하거나 ONTAP 9.17.1 및 이전 버전에서 FlexGroup 볼륨에서 ARP를 실행하는 경우 모드를 전환해야 합니다.
...
ARP/AI 보호 기능과 함께 ARP를 사용하는 경우 ARP가 자동으로 활성화됩니다. 학습 기간이 필요하지 않습니다.
...
ONTAP 9.10.1~9.15.1( FlexGroup 볼륨이 있는 ONTAP 9.17.1 이하)의 경우 학습 기간이 완료된 후 시스템 관리자나 ONTAP CLI를 사용하여 ARP 학습 모드에서 활성 모드로 수동으로 전환할 수 있습니다.

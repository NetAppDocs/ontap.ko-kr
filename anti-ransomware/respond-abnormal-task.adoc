...
.. 다음 중 하나를 수행합니다.
+
*** 파일 유형 경고의 경우 *의심스러운 파일 유형 업데이트 및 지우기*를 선택합니다.
...
NOTE: ONTAP 9.13.1부터 MAV를 사용하여 ARP 설정을 보호하는 경우, 명백한 의심 작업에서는 한 명 이상의 추가 관리자의 승인을 얻도록 요청합니다. link:../multi-admin-verify/request-operation-task.html["모든 관리자로부터 승인을 받아야 합니다"] MAV 승인 그룹과 연관되지 않으면 작업이 실패합니다.
...
. 9.18.1부터 다음 상태를 확인할 수 있습니다. `clear-suspect` 작업:
+
[source, cli]
----

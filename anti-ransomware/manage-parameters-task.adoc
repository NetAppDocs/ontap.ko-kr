...
NOTE: 오탐 경고 발생률을 줄이려면 *Storage > Volumes > Security > Configure workload characteristics*로 이동하여 *Monitor new file types*를 비활성화하십시오. 이 설정은 ONTAP 9.14.1 P7, 9.15.1 P1, 9.16.1 이상 버전에서 기본적으로 비활성화되어 있습니다. ONTAP 9.18.1 P2부터 System Manager는 기준선 및 급증 통계를 포함한 ARP 워크로드 특성을 더 이상 표시하지 않습니다. 급증 통계는 고급 분석을 위해 ONTAP CLI 및 REST API 옵션을 사용하여 계속 사용할 수 있습니다.

...
ONTAP 9.19.1 및 ONTAP Mediator 1.12부터 동일한 ONTAP Mediator 인스턴스를 사용하여 4노드 MetroCluster IP 구성과 SnapMirror 액티브 동기화 클러스터 피어 관계를 동시에 모니터링할 수 있습니다. 구성은 다음 요구 사항을 충족해야 합니다.

* ONTAP Mediator 1.12.1 이상 버전이 2코어 CPU를 탑재한 호스트에 설치되어 있습니다.
* ONTAP Mediator 인스턴스는 HTTPS 프로토콜로 구성되어 있습니다.
+

NOTE: 동일한 ONTAP Mediator 인스턴스를 사용하여 4노드 MetroCluster IP 구성 하나와 SnapMirror 활성 동기화 피어 관계 하나만 모니터링할 수 있습니다.



ONTAP 9.17.1부터 다음을 구성할 수 있습니다. link:cloud-mediator-config-task.html["ONTAP 클라우드 중재자"] SnapMirror Active Sync 구성에서 클러스터의 상태를 모니터링하려면 두 개의 Mediator를 동시에 사용할 수 없습니다.


IMPORTANT: SnapMirror 활성 동기화 및 ONTAP Mediator 또는 ONTAP Cloud Mediator를 ONTAP 9.17.1과 함께 사용하는 경우, 이러한 구성에 대한 중요한 정보는 link:https://library.netapp.com/ecm/ecm_download_file/ECMLP2492508["ONTAP 릴리즈 노트"^]의 *알려진 문제 및 제한 사항*을 참조하십시오.

.ONTAP 중재자 전제 조건
* ONTAP Mediator에는 자체적인 필수 구성 요소가 포함되어 있습니다. ONTAP Mediator를 설치하기 전에 이러한 필수 구성 요소를 충족해야 합니다.
+
자세한 내용은 다음을 참조하세요. link:https://docs.netapp.com/us-en/ontap-metrocluster/install-ip/concept_mediator_requirements.html["ONTAP Mediator 서비스 설치를 준비하세요"^] .
...
ONTAP Mediator를 사용하여 SnapMirror 활성 동기화 관계를 모니터링할 때, 유효한 자체 서명 인증서 또는 인증 기관(CA) 인증서 없이는 ONTAP에서 ONTAP Mediator를 초기화할 수 없습니다. 피어링된 클러스터의 인증서 저장소에 유효한 인증서를 추가해야 합니다.
...
-----END CERTIFICATE-----
----
. 기본 또는 보조 클러스터에 루트 및 중간 CA 인증서를 설치합니다.
+
.. 루트 CA 인증서를 설치합니다.
+
...
+
메시지가 나타나면 다음 예시와 같이 `root.crt`을(를) 입력하십시오.
...
----
. 피어링된 클러스터에 루트 및 중간 CA 인증서를 설치하려면 이전 단계를 반복하십시오.
. 생성된 인증서 이름을 사용하여 설치된 자체 서명 CA 인증서를 확인합니다.
+

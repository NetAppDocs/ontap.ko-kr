...
재해 복구 구성의 상태를 테스트하거나 기본 클러스터에 대한 유지 관리를 수행하기 위해 계획된 페일오버를 수행해야 할 수도 있습니다.
...
. 장애 조치 상태를 모니터링하려면 ONTAP CLI에서  `snapmirror failover show` 명령을 사용하십시오.
...
. 타겟 클러스터에서 페일오버 진행 상황을 모니터링하십시오.
+

...
SnapMirror 활성 동기화 SAN 구성이 있는 클러스터의 일관성 그룹 제한은 관계를 기반으로 계산되며 사용 중인 ONTAP 버전에 따라 달라집니다. 제한은 플랫폼에 독립적입니다.
...
SAN 구성에서 SnapMirror 활성 동기화를 사용할 때 일관성 그룹당 최대 볼륨 수는 플랫폼에 관계없이 동일합니다.
...
SAN 구성에서 SnapMirror 액티브 동기화의 볼륨 제한은 관계 수가 아닌 엔드포인트 수를 기준으로 계산됩니다. 12개의 볼륨으로 구성된 일관성 그룹은 기본 및 보조 클러스터 모두에서 12개의 엔드포인트를 생성합니다. SnapMirror 액티브 동기화와 SnapMirror 동기식 관계 모두 총 엔드포인트 수에 포함됩니다.


IMPORTANT: 이러한 제한 사항은 AFF 및 ASA 시스템에 적용됩니다. ASA r2 시스템(ASA A1K, ASA A90, ASA A70, ASA A50, ASA A30 또는 ASA A20)을 사용하는 경우 link:https://docs.netapp.com/us-en/asa-r2/data-protection/manage-consistency-groups.html["ASA r2 설명서"^]을 참조하십시오.
...
| 4 x 4 SnapMirror 액티브 동기화 솔루션당 고유 LUN, 네임스페이스 또는 스토리지 유닛의 수(ONTAP 9.16.1부터 사용 가능) | 6,144 
...
ONTAP 9.19.1부터 SnapMirror 액티브 싱크는 NAS 프로토콜을 지원합니다. NAS 객체 제한 사항은 다음 표에 나와 있습니다.
...
| HA 쌍당 애플리케이션 일관성 그룹 수 | 지원되지 않습니다 

...
A-시리즈, C-시리즈
 a| 
자동 페일오버 듀플렉스(대칭 Active/Active), 자동 페일오버(비대칭 Active/Passive)
 a| 
* 2 (ONTAP 9.9.1 이상)
...
* ONTAP 9.15.1 이전 릴리스에서는 읽기-쓰기 대상 볼륨(비대칭 액티브-패시브에서 DP에서 읽기-쓰기로 변환된 볼륨)에서 SnapMirror 액티브 동기화 관계가 지원되지 않습니다. 읽기-쓰기 볼륨을 사용하려면 먼저 볼륨 레벨 SnapMirror 관계(비동기 또는 동기)를 생성한 다음 해당 관계를 삭제하여 DP 볼륨으로 변환해야 합니다. 자세한 내용은 link:convert-active-sync-task.html["기존 SnapMirror 관계를 SnapMirror 활성 동기화로 변환"]을(를) 참조하십시오.

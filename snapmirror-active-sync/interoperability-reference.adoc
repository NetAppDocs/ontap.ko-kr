...
NAS 구성에 대한 추가 지원 가능성 정보는 link:protect-nas-task.html["ONTAP SnapMirror 액티브 싱크를 사용하여 NAS 보호"]을 참조하십시오.
...
* AIX
+
** ONTAP 9.19.1부터 Transparent Application Failover(TAF)는 Symmetric active/active 구성에서 지원됩니다. PVR은 필요하지 않습니다.
** ONTAP 9.11.1부터 ONTAP 9.18.1까지 AIX는 대칭형 액티브/액티브 구성의 자동 페일오버를 위해 PVR에서만 지원됩니다.
...
ONTAP 9.19.1부터 AIX용 투명 애플리케이션 페일오버가 대칭형 액티브/액티브 구성에서 SnapMirror 액티브 동기화와 함께 지원됩니다. System Manager 또는 ONTAP CLI를 사용하여 AIX LUN을 생성할 수 있습니다. 투명 페일오버를 보장하려면 ONTAP CLI를 사용할 때 lun create 또는 lun modify 명령과 함께 access-mode local 옵션을 사용해야 합니다. 기본적으로 액세스 모드는 global로 설정되어 있습니다. System Manager를 사용할 때는 호스트로 AIX를 선택하고 액세스 모드를 local로 선택하십시오. LUN 생성에 대한 자세한 내용은 link:../san-admin/provision-storage.html["SAN 스토리지 용량 할당"]를 참조하십시오. AIX 지원에 대한 추가 정보는 NetApp 기술 자료 문서 link:https://kb.netapp.com/on-prem/ontap/DP/SnapMirror/SnapMirror-KBs/SnapMirror_active_sync_support_for_AIX%3A_behavior_by_ONTAP_release_and_migration_guidance["AIX용 SnapMirror 액티브 동기화 지원: ONTAP 릴리스별 동작 및 마이그레이션 지침"^]를 참조하십시오.

ONTAP 9.11.1부터 ONTAP 9.18.1까지 사용 중인 경우, 다음 조건이 이해된다는 동의 하에 표준 엔지니어링 기능 정책 변경 요청(FPVR)을 통해 SnapMirror 액티브 동기화에서 AIX가 지원됩니다.
...
격리된 기본 클러스터에서 발생하는 자동 예기치 않은 장애 조치(AUFO) 이벤트는 기본 클러스터와 보조 클러스터 간의 연결이 끊어지고 기본 클러스터와 중재자 간의 연결도 끊어지는 이중 이벤트 장애로 인해 발생할 수 있습니다. 이는 다른 AUFO 이벤트와 달리 드물게 발생하는 이벤트입니다.
...
SnapMirror 활성 동기화 환경에서 계획되지 않은 사이트 장애 조치(failover) 전환이 발생할 때 Solaris 클라이언트 애플리케이션이 중단되지 않도록 하려면 기본 Solaris OS 설정을 수정합니다. 권장 설정으로 Solaris를 구성하려면 다음을 참조하세요.link:https://kb.netapp.com/Advice_and_Troubleshooting/Data_Protection_and_Security/SnapMirror/Solaris_Host_support_recommended_settings_in_SnapMirror_Business_Continuity_(SM-BC)_configuration["NetApp 지식 기반: SnapMirror Active Sync에서 Solaris 호스트 지원 권장 설정"^] .
...
ONTAP 9.19.1부터 ARP/AI는 SnapMirror 활성 동기화 SAN 관계에 참여하는 기본 볼륨을 지원합니다. 자세한 내용은 link:../anti-ransomware/use-cases-restrictions-concept.html#interoperability-and-vm-considerations["자율 랜섬웨어 보호 사용 사례 및 고려사항"]을 참조하십시오.
...
SnapMirror active sync for SAN은 xref:../data-protection/supported-deployment-config-concept.html[팬아웃 구성] `MirrorAllSnapshots` 정책을 통해 팬아웃 구성을 지원하며, ONTAP 9.11.1부터는  `MirrorAndVault` 정책도 지원합니다. 팬아웃 구성은  `XDPDefault` 정책을 사용하는 SnapMirror active sync에서 지원되지 않습니다.

ONTAP 9.15.1 버전부터 SnapMirror 액티브 싱크는 페일오버 이벤트 후 팬아웃 레그의 자동 재구성을 지원합니다. 기본 사이트에서 보조 사이트로의 페일오버가 성공하면 비동기 팬아웃 레그의 다음 예약된 업데이트 중에 팬아웃 관계 재구성이 시작됩니다. SnapMirror 액티브 싱크 페일오버 직후에는 즉시 트리거되지 않습니다. 비동기 팬아웃 레그는 SAN 구성의 일관성 그룹 관계이거나 독립 볼륨 관계일 수 있습니다. 두 경우 모두 재구성이 가능합니다. 재구성은 계획된 페일오버 또는 계획되지 않은 페일오버에 의해 트리거됩니다. 재구성은 기본 사이트로의 페일백 시에도 발생합니다.
...
.부분 파일 SnapRestore
ONTAP 9.12.1 버전부터 link:../data-protection/restore-part-file-snapshot-task.html["부분 LUN 복원"]은(는) SnapMirror 액티브 동기화 볼륨에 대해 지원됩니다. SnapMirror 액티브 동기화 소스(볼륨)와 대상(스냅샷) 볼륨 간에 복제된 애플리케이션에서 생성한 스냅샷에서 데이터를 복원할 수 있습니다. 동일한 LUN에 여러 데이터베이스가 저장된 호스트에서 데이터베이스를 복원해야 하는 경우 부분 LUN 또는 파일 복원이 필요할 수 있습니다. 이 기능을 사용하려면 데이터의 시작 바이트 오프셋과 바이트 수를 알아야 합니다.

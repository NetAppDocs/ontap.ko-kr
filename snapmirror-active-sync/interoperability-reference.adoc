...
ONTAP 9.15.1 버전부터 SnapMirror 액티브 싱크는 페일오버 이벤트 후 팬아웃 레그의 자동 재구성을 지원합니다. 기본 사이트에서 보조 사이트로의 페일오버가 성공하면 3차 사이트는 보조 사이트를 소스로 처리하도록 자동으로 재구성됩니다. 비동기 팬아웃 레그는 SAN 구성의 일관성 그룹 관계 또는 독립 볼륨 관계일 수 있습니다. 두 경우 모두 재구성이 가능합니다. 재구성은 계획된 페일오버 또는 계획되지 않은 페일오버에 의해 트리거됩니다. 기본 사이트로의 페일백 시에도 재구성이 발생합니다.

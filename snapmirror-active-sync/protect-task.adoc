...
---
= ONTAP SnapMirror 액티브 동기화를 사용하여 SAN 보호
:hardbreaks:
:toclevels: 1
:allow-uri-read: 
:toclevels: 1
...
SnapMirror 액티브 동기화를 사용하여 SAN 구성에 대한 비대칭 보호를 구성하려면 ONTAP 소스 클러스터에서 LUN을 선택하고 이를 일관성 그룹에 추가해야 합니다.
...
* 소스 및 타겟 클러스터는 MetroCluster 구성의 일부가 될 수 없습니다.
...
시스템 관리자 또는 ONTAP CLI를 사용하여 SAN 구성에 대한 대칭 보호를 설정할 수 있습니다. 두 인터페이스 모두에서 xref:index.html#key-concepts[균일 및 비균일 설정]에 대한 단계가 다릅니다.
...
--
.균일 설정에 대한 단계
. 기본 사이트에서 link:../consistency-groups/configure-task.html#create-a-consistency-group-with-new-luns-or-volumes["새 LUN을 사용하여 일관성 그룹 생성"].
+
...
.시스템 관리자
--
. 기본 사이트에서 link:../consistency-groups/configure-task.html#create-a-consistency-group-with-new-luns-or-volumes["새 볼륨 또는 NVMe 네임스페이스를 사용하여 일관성 그룹을 생성합니다"].

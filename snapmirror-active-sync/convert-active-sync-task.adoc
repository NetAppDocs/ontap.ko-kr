...
---
= 기존 ONTAP SnapMirror SAN 관계를 SAN 관계에 대한 SnapMirror 액티브 싱크로 변환합니다
:hardbreaks:
:allow-uri-read: 
:nofooter: 
:icons: font
...
[role="lead"]
SnapMirror 보호 기능을 구성한 경우 SAN 관계를 SnapMirror 액티브 동기화(SAN용)로 변환할 수 있습니다. ONTAP 9.15.1 버전부터는 대칭형 액티브/액티브 보호 기능을 사용하도록 관계를 변환할 수 있습니다.

SVM-DR 관계를 NAS용 SnapMirror 활성 동기화 관계로 변환하는 방법에 대한 자세한 내용은 link:protect-nas-task.html#convert-an-svm-dr-relationship-to-a-snapmirror-active-sync-relationship["SVM DR 관계를 SnapMirror 활성 동기화 관계로 변환합니다"]을 참조하십시오.
...
* link:mediator-install-task.html["ONTAP 중재자"] SnapMirror Active Sync에 맞게 구성해야 합니다.

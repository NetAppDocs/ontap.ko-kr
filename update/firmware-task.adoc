...
summary: 디스크, 디스크 셸프, 서비스 프로세서 또는 Baseboard Management Controller와 같이 클러스터에서 지원되는 장치에 펌웨어 업데이트를 적용할 수 있습니다. 
---
= 펌웨어를 수동으로 업데이트합니다
:hardbreaks:
:toclevels: 1
...
[role="lead"]
ONTAP 9.9.1부터 link:https://docs.netapp.com/us-en/active-iq/["디지털 자문업체"^]에 등록된 경우 System Manager에서 디스크, 디스크 셸프, SP(서비스 프로세서) 또는 BMC(Baseboard Management Controller)와 같은 지원되는 장치에 대한 펌웨어 업데이트가 클러스터에서 보류 중일 때 알림을 받을 수 있습니다.

ONTAP 9.8을 사용 중이거나 Digital Advisor에 등록되어 있지 않은 경우 NetApp 지원 사이트로 이동하여 펌웨어 업데이트를 다운로드하십시오.
...
.단계
사용 중인 ONTAP 버전과 Digital Advisor 등록 여부에 따라 적절한 절차를 따르십시오.

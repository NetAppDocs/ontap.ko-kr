---
permalink: update/enable-automatic-updates-task.html 
sidebar: sidebar 
keywords: ontap, enable, upgrade, update, automatic, automated, firmware, ransomware, bmc, sp, system, timezone database, timezone 
summary: 자동 업데이트를 활성화하여 ONTAP가 사용자의 개입 없이 펌웨어, 시스템 및 보안 업데이트를 다운로드하고 설치할 수 있도록 합니다. 
---
= 자동 업데이트를 활성화합니다
:allow-uri-read: 
:icons: font
:imagesdir: ../media/


[role="lead"]
System Manager를 사용하여 자동 업데이트를 활성화하고 ONTAP가 사용자의 개입 없이 펌웨어, 시스템 및 보안 업데이트를 다운로드하고 설치할 수 있도록 합니다.

ONTAP CLI를 사용하여 SP/BMC 펌웨어 업데이트만 관리할 수 있습니다.

* link:../system-admin/methods-manage-sp-bmc-firmware-updates-concept.html["ONTAP SP/BMC 펌웨어 업데이트 관리"]에 대해 자세히 알아보십시오.
* System Manager와 CLI를 사용한 자동 업데이트 관리 방식의 차이점에 대한 자세한 내용은 기술 자료 문서 link:https://kb.netapp.com/on-prem/ontap/Ontap_OS/OS-KBs/Why_do_GUI_and_CLI_show_different_status_for_ONTAP_automatic_updates_configuration["GUI와 CLI에서 ONTAP 자동 업데이트 구성에 대해 서로 다른 상태가 표시되는 이유는 무엇입니까?"]를 참조하십시오.


각 시스템 구성 요소에 대한 자동 업데이트의 가용성은 ONTAP 버전에 따라 다릅니다.

[cols="25,75"]
|===
| ONTAP 버전입니다 | 사용 가능한 자동 업데이트 


| 9.16.1 이상  a| 
* 인공 지능(ARP/AI)을 이용한 자율적 랜섬웨어 방어
* ONTAP 시간대 데이터베이스
* 스토리지 장치, 디스크 및 디스크 쉘프를 위한 스토리지 펌웨어
* 서비스 프로세서 및 BMC 모듈용 SP/BMC 펌웨어
* 디스크 자격 평가 패키지(DQP)




| 9.13.1 이상  a| 
* ONTAP 시간대 데이터베이스
* 스토리지 장치, 디스크 및 디스크 쉘프를 위한 스토리지 펌웨어
* 서비스 프로세서 및 BMC 모듈용 SP/BMC 펌웨어
* 디스크 자격 평가 패키지(DQP)




| 9.10.1 이상  a| 
* 스토리지 장치, 디스크 및 디스크 쉘프를 위한 스토리지 펌웨어
* 서비스 프로세서 및 BMC 모듈용 SP/BMC 펌웨어
* 디스크 자격 평가 패키지(DQP)


|===
사용 중인 ONTAP 버전에 대해 자동 업데이트를 사용할 수 없거나 자동 업데이트를 사용하도록 설정하지 않은 경우 펌웨어, 시간대 데이터베이스 및 보안 업데이트를 수동으로 수행할 수 있습니다.

.이 작업에 대해
자동 업데이트를 활성화하려면 EULA 약관에 동의하고 사용자 환경에 맞게 각 시스템 구성 요소의 기본 업데이트 동작을 확인하거나 구성해야 합니다.

ONTAP 9.16.1 이하에서 생성된 후 9.16.1 이상으로 업그레이드된 클러스터의 경우:

* 업그레이드 전에 자동 업데이트 기능이 활성화된 경우 시스템 구성 요소 업데이트 기본값은 업그레이드 후 변경되지 않습니다.
* 업그레이드 전에 자동 업데이트가 활성화되어 있지 않으면 ONTAP 9.16.1 이상의 시스템 구성 요소 업데이트 기본값이 적용됩니다.


자동 시스템 구성 요소 업데이트 기본값은 ONTAP 버전에 따라 자동으로 업데이트되거나 알림을 표시하도록 설정되어 있습니다.  절차를 완료하기 전에 이러한 설정이 사용자 환경에 맞는지 확인하세요.

다음은 https://www.youtube.com/watch?v=GoABILT85hQ["비디오"^] 자동 업데이트 프로세스 사용에 대한 간략한 개요를 보여 줍니다.

.시작하기 전에
* 현재 지원 권한이 있어야 합니다. 시스템 세부 정보 * 페이지에서 확인할 수 link:https://mysupport.netapp.com/site/["NetApp Support 사이트"^]있습니다.
* 자동 업데이트를 사용하려면 먼저 HTTPS가 포함된 AutoSupport를 활성화해야 합니다. 클러스터에서 AutoSupport가 활성화되어 있지 않거나 클러스터에서 다른 전송 프로토콜을 사용하여 AutoSupport가 활성화되어 있는 경우, 이 절차 중에 HTTPS로 사용하도록 설정할 수 있습니다.



NOTE: AutoSupport OnDemand는 HTTPS 전송 프로토콜을 사용하여 기술 지원 부서로 메시지를 보내도록 구성된 경우 기본적으로 활성화되며 작동합니다.

* ONTAP 9.10.1부터 자동 업데이트를 사용하도록 설정한 경우 다음 추가 URL에 HTTPS 연결이 있는지 확인합니다.
+
** https://support-sg-naeast.netapp.com 으로 문의하십시오
** https://support-sg-nawest.netapp.com 으로 문의하십시오




.단계
[role="tabbed-block"]
====
.System Manager - ONTAP 9.16.1 이상
--
. System Manager에서 * 클러스터 > 설정 * 을 선택합니다.
. *소프트웨어 업데이트*에서 *사용*을 선택합니다.
. *자동 업데이트 설정*에서 각 업데이트 유형에 대해 수행할 작업을 선택하십시오.
+
모든 옵션에 대해 기본적으로 *자동 업데이트*가 선택되어 있습니다. 기본 설정을 *알림 표시* 또는 *업데이트 자동 해제*로 변경할 수 있습니다.

. *현재 및 향후 모든 업데이트에 동의합니다*를 선택한 다음 *저장*을 선택합니다.


--
.시스템 관리자 - ONTAP 9.15.1 이하
--
. System Manager에서 * 클러스터 > 설정 * 을 선택합니다.
. *자동 업데이트*에서 image:icon_kabob.gif["메뉴 선택"]를 선택한 다음 *활성화*를 선택합니다.
+
모든 옵션에 대해 기본적으로 *자동 업데이트*가 선택되어 있습니다. 기본 설정을 *알림 표시* 또는 *업데이트 자동 해제*로 변경할 수 있습니다.

. *현재 및 향후 모든 업데이트에 동의합니다*를 선택한 다음 *저장*을 선택합니다.


--
.CLI를 참조하십시오
--
ONTAP CLI를 사용하여 SP/BMC 펌웨어 업데이트만 관리할 수 있습니다

. 자동 펌웨어 업데이트 활성화:
+
[source, cli]
----
system service-processor image modify -node <node_name> -autoupdate true
----


--
====
.관련 정보
* link:firmware-task.html["펌웨어 업데이트를 수동으로 수행하는 방법을 알아봅니다"]
* link:https://kb.netapp.com/Advice_and_Troubleshooting/Data_Storage_Software/ONTAP_OS/How_to_update_time_zone_information_in_ONTAP_9["기술 자료 문서, ONTAP 9에서 표준 시간대 정보를 업데이트하는 방법"^]
* link:../anti-ransomware/enable-arp-ai-with-au.html["수동으로 보안 업데이트를 수행하는 방법을 알아봅니다"]
* link:../system-admin/requirements-autosupport-reference.html["AutoSupport 사용을 준비합니다"]
* link:../system-admin/troubleshoot-autosupport-https-task.html["HTTPS를 통한 AutoSupport 메시지 전달 문제를 해결합니다"]




== 비디오: 자동 펌웨어 업데이트

ONTAP 9.10.1부터 제공되는 자동 펌웨어 업데이트 기능을 살펴보십시오.

video::GoABILT85hQ[youtube,width=848,height=480]
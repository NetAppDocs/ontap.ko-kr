...
[role="lead"]
NetApp에서는 새 클러스터를 생성할 때 System Manager를 사용할 것을 권장합니다. System Manager는 클러스터 설정을 위한 간단하고 쉬운 워크플로를 제공합니다. MetroCluster 구성에서 ONTAP 9.7 이하 버전을 실행 중이거나 지원되는 시스템에서 IPv6 전용 클러스터를 구성해야 하는 경우에만 ONTAP 명령줄 인터페이스(CLI)를 사용해야 합니다.
...
클러스터 설정 중 관리 LIF에 IPv6 주소를 할당하는 기능은 다음 시스템 및 ONTAP 버전에서 지원됩니다.
...
클러스터 설정 중 IPv6 주소 할당을 지원하려면 BMC 버전이 19.3 이상이어야 합니다.
...
|===
지원되는 시스템에서 IPv6 주소를 할당하려면 ONTAP CLI를 사용하여 클러스터 설정을 수행해야 합니다. 시스템 및 ONTAP 버전에서 클러스터 설정 중 IPv6 주소 할당이 지원되지 않는 경우, System Manager를 사용하여 IPv4 주소로 클러스터를 생성한 다음 클러스터 설정을 완료한 후 link:convert-ipv4-to-ipv6-task.html["IPv6로 변환합니다"] 주소를 할당해야 합니다.


NOTE: System Manager는 ONTAP 9.6 이하 버전에서 IPv6 네트워킹이 필요한 배포를 지원하지 않습니다.

.시작하기 전에
* 시스템 모델의 설치 및 설정 지침에 따라 새 스토리지 시스템을 설치하고 케이블을 연결하고 전원을 켜야 합니다.
+

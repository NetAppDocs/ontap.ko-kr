...
---
= ONTAP SnapMirror 소스 또는 대상 볼륨에서 FlexClone 볼륨을 생성하는 방법에 대해 알아보십시오
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
기존 볼륨 SnapMirror 관계의 소스 또는 대상 볼륨에서 FlexClone 볼륨을 생성할 수 있습니다. 그러나 ONTAP 9.12.1 이전 버전의 ONTAP을 사용하고 있고,  `DPDefault` 정책을 사용하는 DP 유형의 관계인 경우, 이렇게 하면 향후 SnapMirror 복제 작업이 성공적으로 완료되지 않을 수 있습니다.
...
ONTAP 9.12.1 이상 버전에서 `MirrorAllSnapshots` 정책을 사용하여 XDP 관계를 사용하는 경우 이 문제가 발생하지 않아야 합니다.

관련 정보: link:../data-protection/convert-snapmirror-version-flexible-task.html["기존 ONTAP SnapMirror DP 유형 관계를 XDP로 변환합니다"]

...
summary: Flash Pool 애그리게이트 또는 HDD 애그리게이트가 있는 FAS 시스템에서 볼륨 또는 애그리게이트 레벨에서 인라인 데이터 컴팩션을 설정합니다. 
---
...
[role="lead"]
Flash Pool(하이브리드) 애그리게이트 또는 HDD 애그리게이트가 있는 FAS 시스템에서 볼륨 단위로 인라인 데이터 압축을 활성화하려면  `volume efficiency` 클러스터 셸 명령을 사용하면 됩니다. FAS 시스템에서 생성된 볼륨의 경우 데이터 압축은 기본적으로 비활성화되어 있습니다.  `volume efficiency`에 대해 자세히 알아보려면 link:https://docs.netapp.com/us-en/ontap-cli/search.html?q=volume+efficiency["ONTAP 명령 참조입니다"^]를 참조하세요.

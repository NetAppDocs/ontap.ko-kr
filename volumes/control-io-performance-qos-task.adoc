...
* ONTAP 9.18.1부터 SVM에 포함된 볼륨과 같이 QoS 정책이 있는 다른 개체 내에 포함된 개체에 QoS 정책을 할당할 수 있습니다.  중첩된 QoS 정책을 사용하면 가장 제한적인 정책이 적용됩니다.
+
ONTAP 9.18.1 이전 릴리스에서는 QoS 정책이 있는 다른 객체 내에 포함된 객체에 QoS 정책을 할당할 수 있지만, 해당 객체는 QoS에 의해 별도로 추적되며 상호 작용하지 않습니다.  예를 들어, SVM의 상한이 50 IOP이고 FlexVol volume 의 상한이 100 IOP인 경우 FlexVol volume 100 IOP의 상한으로 제한되지만 QoS에서는 SVM 작업 부하에 대해 계산되지 않습니다.

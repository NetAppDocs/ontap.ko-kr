...
.단계
. System Manager에서 *스토리지 > 볼륨*을 선택합니다.
+
볼륨 목록이 표시됩니다.

+
SVM 관리자는 제한된 옵션 세트만 볼 수 있으며 볼륨 추가가 허용되지 않을 수 있습니다.
...
볼륨을 추가합니다
 a| 
image:icon_add_blue_bg.gif["추가 아이콘"]를 선택합니다. link:../task_admin_add_a_volume.html["볼륨을 추가합니다"]을 참조하십시오.
...
* image:icon_delete_with_can_white_bg.gif["삭제 아이콘"]를 선택하여 선택한 볼륨을 삭제합니다.
* 선택한 볼륨에 보호 정책을 할당하려면 image:icon_protect.gif["보호 아이콘"]을(를) 선택합니다.
* image:icon-more-kebab-white-bg.gif["추가 아이콘"]를 선택하여 선택한 모든 볼륨에 대해 수행할 다음 작업 중 하나를 선택합니다.
+
** 할당량을 설정합니다
** 오프라인 상태로 전환
** 이동
...
단일 볼륨 관리
 a| 
볼륨 옆에서 image:icon_kabob.gif["메뉴 옵션 아이콘"]을 선택한 다음, 수행할 다음 작업 중 하나를 선택하십시오.
...
볼륨 이름 옆의 image:icon-edit-pencil-blue-outline.png["편집 아이콘"]을(를) 선택한 다음 볼륨 이름을 수정합니다.

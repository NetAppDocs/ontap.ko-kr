...
[role="lead"]
ONTAP 9.10.1부터 볼륨 수준 스토리지 효율성 모드인 _default_ 및 _efficient_는 AFF 시스템에서만 지원됩니다. 이러한 모드에서는 새로운 AFF 볼륨을 생성할 때 기본 모드인 파일 압축(기본값)과 온도에 민감한 저장 효율성(효율적) 중에서 선택할 수 있습니다. 이 모드에서는 온도에 민감한 저장 효율성을 활성화합니다.

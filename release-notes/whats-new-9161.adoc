...
xref:../san-admin/enable-space-allocation.html[NVMe 공간 할당 취소가 기본적으로 활성화되어 있습니다]
 a| 
NVMe 네임스페이스의 경우 공간 할당 해제("홀 펀칭" 및 "언맵"이라고도 함)가 기본적으로 활성화되어 있습니다. 공간 할당 해제를 통해 호스트는 네임스페이스에서 사용되지 않은 블록을 할당 해제하여 공간을 회수할 수 있습니다. 이를 통해 전반적인 저장 효율성이 크게 향상되며, 특히 데이터 회전율이 높은 파일 시스템의 경우 더욱 그렇습니다.
...
xref:../anti-ransomware/index.html[AI 개선(ARP/AI)을 통한 자율적 랜섬웨어 방어]
 a| 
자율형 랜섬웨어 보호 기능이 새로운 AI 기능으로 업그레이드되어 99%의 정확도와 재현율로 랜섬웨어 공격을 탐지하고 대응할 수 있게 되었습니다. AI는 포괄적인 데이터 세트로 학습되었기 때문에 FlexVol 볼륨에서 실행되는 ARP/AI는 더 이상 학습 기간이 필요하지 않으며 즉시 활성 모드로 작동합니다. 또한 ARP/AI는 최신 위협에 대한 지속적인 보호 및 복원력을 보장하기 위해 자동 업데이트 기능을 제공합니다.

...
| 자율형 랜섬웨어 보호(ARP/AI) | 지원되는 플랫폼에서는 12시간의 유예 기간 후 모든 새 볼륨에 대해 ARP/AI가 기본적으로 자동으로 활성화됩니다. | ONTAP 9.18.1 
...
| ONTAP SVM에 대한 캐시된 긍정적 자격 증명의 TTL(Time-to-Live) | ONTAP 9.17.1 버전부터 ONTAP SVM에 대한 캐시된 긍정 자격 증명의 TTL(Time-to-Live)이 24시간(86,400,000밀리초)에서 1시간(3,600,000밀리초)으로 단축되었습니다. | ONTAP 9.17.1 
...
| 로드 공유 미러 | 부하 공유 미러 관계를 생성할 때 SVM에서 저장 용량 제한을 활성화할 수 없습니다. | ONTAP 9.16.1 
...
| SnapMirror 클라우드 | SnapMirror 클라우드에서 지원하는 S3 버킷 수를 6개에서 100개로 늘렸습니다. | ONTAP 9.19.1 


| S3 연결 흐름 제어 제한 | 메모리가 32GB 이하인 클라우드 인스턴스의 경우 ONTAP S3 연결에 대한 흐름 제어 제한이 2MB에서 256KB로 감소했습니다. | ONTAP 9.19.1 


| S3 유휴 연결 시간 초과 | S3 유휴 연결 시간 초과가 20분에서 5분으로 단축되었습니다. | ONTAP 9.19.1 
...
| 클러스터 피어링 | 지원되는 클러스터 피어 제한을 255개에서 400개로 늘려 클러스터 피어 관계가 많은 환경의 확장성을 향상시켰습니다. | ONTAP 9.14.1 
...
* FlexGroup의 최대 구성 요소 수는 24개를 초과할 수 없습니다.
* SVM-DR의 볼륨 제한은 노드당 500, HA 쌍당 1000입니다(FlexVol 볼륨 및 FlexGroup 구성 요소 포함).
+

NOTE: 제한 사항은 플랫폼별로 다릅니다. link:https://hwu.netapp.com["NetApp Hardware Universe를 참조하십시오"^]로 이동하여 플랫폼과 ONTAP 버전을 선택하여 구성에 맞는 올바른 제한 사항을 확인하십시오.
...
| SnapMirror 동시 전송 | 비동기 볼륨 수준 동시 전송의 최대 수가 100에서 200으로 증가했습니다.  SnapMirror 클라우드 전송은 하이엔드 시스템에서는 32개에서 100개로, SnapMirror 엔드 시스템에서는 6개에서 20개로 증가했습니다. | ONTAP 9.8 

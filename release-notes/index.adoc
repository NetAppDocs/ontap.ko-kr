...
*  https://docs.netapp.com/us-en/ontap-afx/release-notes/whats-new-9191.html["NetApp AFX 스토리지 시스템을 위한 새롭고 향상된 ONTAP 소프트웨어 기능"^]에 대해 알아보세요.
*  https://docs.netapp.com/us-en/asa-r2/release-notes/whats-new-9191.html["NetApp ASA r2 스토리지 시스템을 위한 새롭고 향상된 ONTAP 소프트웨어 기능"^]에 대해 알아보세요.
...
ONTAP 9.19.1은 데이터 인프라 현대화를 지원하기 위해 보안, 확장성 및 스토리지 관리 분야에서 새롭고 향상된 기능을 제공합니다. 이러한 향상된 기능을 통해 확장성을 높이고 운영 효율성을 개선하며 미래를 위한 데이터 보안을 강화할 수 있습니다. 새로운 기능 및 향상된 기능의 전체 목록은 xref:whats-new-9191.adoc[ONTAP 9.19.1의 새로운 기능]을 참조하십시오.

* xref:../performance-admin/guarantee-throughput-qos-task.html[구성 가능한 QoS 버스트 정책]
+
구성 가능한 QoS 버스트 정책을 통해 QoS 제한을 초과하는 제어된 시간 제한 성능 버스트를 허용하여 공정성을 저해하거나 과도한 프로비저닝을 요구하지 않고도 피크 이벤트 동안 워크로드 응답성을 향상시킬 수 있습니다.

* xref:../nfs-admin/enable-nfs-over-tls-task.html[TLS 1.3을 통한 NFS]
+
표준 기반 TLS 1.3 암호화로 전송 중인 NFS 데이터를 보호하여 엔터프라이즈 규모에서 암호화된 NFS 도입의 보안을 강화하고 간소화합니다. 

* xref:../s3-config/regenerate-access-keys.html[사용자당 여러 개의 S3 액세스 키]
+
사용자별로 여러 개의 활성 S3 액세스 키를 지원하여 애플리케이션 다운타임 없이 원활한 자격 증명 교체를 가능하게 하고, 고객이 객체 워크로드에 대한 중단 없는 액세스를 유지하면서 보안 및 규정 준수를 강화할 수 있도록 지원합니다.

* xref:../snapmirror-active-sync/protect-nas-task.html[SnapMirror NAS용 활성 동기화]
+
NAS 지원을 통해 SnapMirror 액티브 싱크는 이제 투명한 페일오버 기능을 갖춘 제로 RPO(복구 목표 시점)를 제공하는 통합 비즈니스 연속성 솔루션이 되었습니다. 

* xref:../san-config/configure-fc-nvme-hosts-ha-pairs-reference.html[FC 직접 연결]
+
안전한 FC HBA 카드 지원을 도입하여 신뢰할 수 있는 호스트 연결을 가능하게 하고, 성능을 유지하면서 엔터프라이즈 규정 준수 요구 사항을 지원하는 동시에 하드웨어 기반의 전송 중 FC 보안을 위한 기반을 구축합니다. 

* xref:../san-admin/multipath-automatic-lif-failover-support.html[통합 액티브-액티브 LIF 페일오버]
+
AFF 시스템은 자동 LIF 페일오버를 지원하는 액티브-액티브 다중 경로를 제공하여 모든 SAN 경로를 활성 최적화 상태로 유지함으로써 페일오버 중에도 호스트 I/O가 즉시 지속되도록 합니다. 

* link:https://docs.netapp.com/us-en/ontap-fli/get-started/learn-about-data-migration.html["FLI의 iSCSI 지원"^]
+
외부 LUN 가져오기(FLI)에 대한 iSCSI 지원을 추가하여 기존 iSCSI 연결을 사용하여 최소한의 중단으로 타사 스토리지 시스템의 블록 데이터를 ONTAP으로 마이그레이션할 수 있습니다. 

* xref:../s3-config/create-modify-bucket-policy-task.html[S3 조건부 쓰기 및 삭제]
+
S3 API를 사용하여 조건부 쓰기 및 삭제를 지원하므로 애플리케이션에서 객체 업데이트 및 삭제 중에 데이터 일관성 검사를 시행할 수 있어 동시 및 분산 객체 워크로드에서 실수로 덮어쓰거나 삭제될 위험을 줄일 수 있습니다.

* xref:../concepts/path-failover-concept.html#nas-path-failover[LIF 장애 조치 통합]
+
...
* xref:../networking/improve-tcp-wan-performance.html[향상된 WAN 성능]
+
TCP RACK을 통한 향상된 WAN 성능은 혼잡한 네트워크 환경에서 처리량과 복원력을 개선하여 원격 파일 액세스 및 사이트 간 데이터 서비스를 가속화합니다. 

* xref:../networking/configure-tls-offload.html[더 빠른 백엔드 클러스터 암호화를 위한 하드웨어 가속 kTLS]
+
암호화 하드웨어 가속 kTLS를 도입하여 백엔드 클러스터 암호화를 강화하고, 암호화 처리를 지원되는 SmartNIC로 오프로드하여 더 높은 성능과 낮은 CPU 오버헤드를 제공하는 동시에 클러스터 및 HA 트래픽 전반에 걸쳐 항상 안전한 통신을 유지합니다. 

* xref:../anti-ransomware/use-cases-restrictions-concept.html#snapmirror[동기식 복제 워크로드를 위한 AI 기반 랜섬웨어 탐지]
+
SnapMirror 동기식 SAN 및 NAS 환경과 SnapMirror 액티브 동기식 SAN 환경에 자율형 랜섬웨어 보호(ARP/AI) 기능을 확장하여 제로 RPO 아키텍처에서 탐지 및 복구 신호를 유지합니다. 

* xref:../concept-svm-admin-dashboard-sm.html[새로운 ONTAP System Manager 대시보드를 사용하여 SVM을 모니터링하고 관리합니다]
+
테넌트 SVM 관리를 위한 사용자 친화적인 GUI를 제공하여 복잡한 CLI 인터페이스의 필요성을 없애줍니다. 
...
ONTAP 9.18.1은 데이터 인프라 현대화를 지원하기 위해 보안, 확장성, 스토리지 관리 분야에서 새롭고 향상된 기능을 제공합니다.  이러한 개선 사항은 확장하고, 더 효율적으로 운영하고, 미래를 위해 데이터를 보호하는 데 도움이 됩니다.  새로운 기능 및 향상된 기능의 전체 목록을 보려면 다음을 참조하세요.xref:whats-new-9181.adoc[ONTAP 9.18.1의 새로운 기능] .

* xref:../networking/configure-backend-encryption.html[백엔드 클러스터 HA 암호화]
+
백엔드 클러스터 네트워크를 암호화하기 위해 mTLS를 사용할 수 있는 기능입니다.

* xref:../networking/configure_network_security_using_federal_information_processing_standards_fips.html[전송 중인 데이터에 대한 PQC 지원]
+
전송 중인 데이터에 포스트 양자 알고리즘을 사용할 수 있는 능력.

* xref:../networking/ipsec-prepare.html[HW 오프로드를 통한 IPv6 IPSec 지원]
+
IPv6 주소로 하드웨어 오프로드를 통한 IPSec 지원.

* xref:../anti-ransomware/use-cases-restrictions-concept.html#supported-configurations[ARP/AI 지원 FlexGroup 볼륨]
+
NetApp FlexGroup 볼륨은 ARP/AI를 지원하여 고급 머신 러닝 기반 랜섬웨어 탐지 및 보호 기능을 제공합니다.

* xref:../san-admin/nvme-copy-offload.html[NVMe를 위한 복사 오프로드]
+
복사 작업을 스토리지에 직접 오프로드하여 대용량 데이터 전송을 가속화하고, 호스트 CPU와 네트워크 부하를 줄입니다.

* xref:../s3-snapshots/list-restore-s3-snapshot.html[저장소 기반 S3 버킷 스냅샷 복원]
+
...
+
ARP/AI는 암호화 기반 이상 탐지를 통해 SAN 볼륨을 지원하고, 상세한 엔트로피 통계를 위한 새로운 명령어를 도입했으며, 기존에 NAS에 초점을 맞췄던 랜섬웨어 보호 메시지를 System Manager에서 통합했습니다. 구성 가능한 탐지 임계값과 더욱 확실해진 스냅샷 보존 기능을 통해 다양한 워크로드에 더욱 유연하게 대응할 수 있습니다.
...
+
ONTAP 9.16.1은 악의적인 내부 공격으로부터 추가적인 보호를 위해 MAV 프레임워크에 더 많은 명령어를 추가했습니다. 이러한 개선 사항에는 다양한 Consistency Group(CG), VScan 및 Autonomous Ransomware Protection(ARP/AI) 관리, NVMe 구성 명령어가 포함됩니다.

* xref:../anti-ransomware/index.html[AI 개선(ARP/AI)을 통한 자율적 랜섬웨어 방어]
+
자율형 랜섬웨어 보호 기능이 새로운 AI 기능으로 업그레이드되어 99%의 정확도와 재현율로 랜섬웨어 공격을 탐지하고 대응할 수 있게 되었습니다. AI는 포괄적인 데이터 세트로 학습되었기 때문에 FlexVol 볼륨에서 실행되는 ARP/AI는 더 이상 학습 기간이 필요하지 않으며 즉시 활성 모드로 작동합니다. 또한 ARP/AI는 ONTAP 업그레이드와 관계없이 자동 업데이트 기능을 도입하여 최신 위협에 대한 지속적인 보호 및 복원력을 보장합니다.
...
+
ONTAP 9.15.1 버전부터는 심볼 링크가 가리키는 데이터 대신 심볼 링크 자체를 백업하는 옵션이 제공됩니다. 이 기능은 백업 애플리케이션의 성능 향상을 비롯한 여러 이점을 제공합니다. ONTAP CLI 또는 ONTAP REST API를 사용하여 이 기능을 활성화할 수 있습니다.
...
** xref:../anti-ransomware/support-arpai.html#multi-admin-verification-with-arp-ai-protected-volumes[다중 관리자 인증 지원]
+
...
+
FlexGroup 구성원 간의 용량 불균형이 발생할 경우, FlexGroup는 ONTAP CLI, ONTAP REST API 및 System Manager를 통해 중단 없이 재조정하고 관리할 수 있습니다. 최적의 성능을 위해서는 FlexGroup 내의 구성원들의 사용 용량이 균등하게 분산되어야 합니다.

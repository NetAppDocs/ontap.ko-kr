...
summary: Storage-Level Access Guard를 구성하는 워크플로는 NTFS 파일 권한 및 감사 정책을 구성하는 데 사용하는 것과 동일한 ONTAP CLI 명령을 사용합니다. 지정된 대상에 대한 파일 및 디렉터리 액세스를 구성하는 대신 지정된 스토리지 가상 머신 볼륨에 대해 SLAG를 구성합니다. 
---

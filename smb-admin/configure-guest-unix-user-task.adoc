...
summary: 게스트 UNIX 사용자 옵션을 구성하면 신뢰할 수 없는 도메인에서 로그인하는 사용자가 게스트 UNIX 사용자로 매핑되어 CIFS 서버에 연결할 수 있습니다. 반대로 신뢰할 수 없는 도메인 사용자의 인증이 실패하도록 하려면 게스트 UNIX 사용자를 구성하지 않아야 합니다. 기본 설정은 신뢰할 수 없는 도메인 사용자의 CIFS 서버 연결을 허용하지 않는 것입니다. 
---

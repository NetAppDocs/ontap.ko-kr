...
summary: ONTAP 9부터 NetBios 네임 서비스는 기본적으로 비활성화되어 있습니다. 이전에는 CIFS를 지원하는 스토리지 가상 머신이 네트워크에서 WINS가 활성화되어 있는지 여부와 관계없이 이름 등록 브로드캐스트를 전송했습니다. NBNS가 필요한 구성으로만 이러한 브로드캐스트를 제한하려면 새 CIFS 서버에 대해 NBNS를 명시적으로 활성화해야 합니다. 
---

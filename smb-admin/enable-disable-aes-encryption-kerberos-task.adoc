...
summary: Kerberos 기반 통신에서 가장 강력한 보안을 활용하려면 SMB 서버에서 AES-256 및 AES-128 암호화를 활성화할 수 있습니다. Active Directory KDC와의 Kerberos 기반 통신에 대해 SMB 서버가 AES 암호화 유형을 선택하지 않도록 하려면 AES 암호화를 비활성화할 수 있습니다. 기본적으로 AES 암호화는 비활성화되어 있습니다. 
---
...
ONTAP 9.12.1부터 AES 암호화는 을 사용하여 활성화 및 비활성화됩니다 `-advertised-enc-types` 옵션: AD KDC에 보급된 암호화 유형을 지정할 수 있습니다. 기본 설정은 입니다 `rc4` 및 `des`그러나 AES 유형이 지정되면 AES 암호화가 활성화됩니다. 이 옵션을 사용하여 약한 RC4 및 DES 암호화 유형을 명시적으로 비활성화할 수도 있습니다. ONTAP 9.11.1 이하 버전에서는 을 사용해야 합니다 `-is-aes-encryption-enabled` AES 암호화를 활성화 및 비활성화하는 옵션과 암호화 유형을 지정할 수 없습니다.
 ...
 `-advertised-enc-types` 매개변수를 수정하면 지정된 암호화 유형이 이전에 구성된 유형을 대체합니다. 이전의 모든 암호화 유형 설정은 삭제됩니다.

보안 강화를 위해 스토리지 가상 머신(SVM)은 AES 보안 옵션이 변경될 때마다 AD(Active Directory)에서 머신 계정 암호를 변경합니다. 암호를 변경하려면 머신 계정이 포함된 조직 단위(OU)에 대한 관리자 AD 자격 증명이 필요할 수 있습니다. 자격 증명 입력 프롬프트는 ONTAP가 Active Directory에서 머신 계정 암호를 재설정해야 하고 AD 구성에서 머신 계정이 포함된 조직 단위에 대한 관리자 자격 증명이 필요한 경우에만 나타납니다. AD가 동기화되어 있고 자동 재설정이 활성화된 경우 프롬프트가 나타나지 않습니다.

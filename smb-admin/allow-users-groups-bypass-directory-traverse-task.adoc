...
summary: 사용자가 특정 파일 경로에 있는 모든 디렉터리를 탐색할 수 있도록 허용하려면, 해당 디렉터리에 대한 권한이 없더라도 탐색이 가능하도록 스토리지 가상 머신의 로컬 SMB 사용자 또는 그룹에 SeChangeNotifyPrivilege 권한을 추가하면 됩니다. 기본적으로 사용자는 디렉터리 탐색 권한 검사를 건너뛸 수 있습니다. 
---

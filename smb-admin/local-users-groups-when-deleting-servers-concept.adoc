...
summary: CIFS 서버가 생성될 때 기본 로컬 사용자 및 그룹 집합이 생성되며, 이들은 CIFS 서버를 호스팅하는 스토리지 가상 시스템과 연결됩니다. SVM 관리자는 언제든지 로컬 사용자 및 그룹을 생성할 수 있습니다. CIFS 서버를 삭제할 때 로컬 사용자 및 그룹에 어떤 일이 발생하는지 알아야 합니다. 
---

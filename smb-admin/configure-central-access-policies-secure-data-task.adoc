...
summary: 중앙 액세스 정책을 사용하여 CIFS 서버의 데이터에 대한 액세스를 보호하려면 몇 가지 단계를 수행해야 합니다. 여기에는 CIFS 서버에서 Dynamic Access Control 활성화, Active Directory에서 중앙 액세스 정책 구성, GPO를 사용하여 Active Directory 컨테이너에 중앙 액세스 정책 적용, CIFS 서버에서 GPO 활성화가 포함됩니다. 
---

...
summary: 자동 위치 지정 기능은 SMB 자동 노드 참조를 사용하여 스토리지 가상 머신에서 SMB 클라이언트 성능을 향상시킵니다. 자동 노드 참조는 요청하는 클라이언트를 데이터가 저장된 볼륨을 호스팅하는 노드 SVM의 LIF로 자동으로 리디렉션하여 클라이언트 응답 시간을 개선할 수 있습니다. 
---

...
summary: NTFS 보안 설명자에 DACL 액세스 제어 항목을 추가하는 것은 파일 또는 폴더에 NTFS ACL을 구성하고 적용하는 두 번째 단계입니다. 각 항목은 어떤 개체에 대한 액세스가 허용되거나 거부되는지를 식별하고, 해당 개체가 ACE에 정의된 파일 또는 폴더에 대해 수행할 수 있는 작업과 수행할 수 없는 작업을 정의합니다. 
---

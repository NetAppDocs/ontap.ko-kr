...
summary: Active Directory 통신 보안을 위해 SSL/TLS를 통한 LDAP를 사용하려면 먼저 Active Directory 인증서 서비스의 자체 서명된 루트 CA 인증서를 인증서 파일로 내보내고 ASCII 텍스트 파일로 변환해야 합니다. 이 텍스트 파일은 ONTAP에서 스토리지 가상 머신에 인증서를 설치하는 데 사용됩니다. 
---

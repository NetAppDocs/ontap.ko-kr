...
summary: 스토리지 가상 머신에 로컬 사용자 및 로컬 그룹을 생성하는 데에는 여러 가지 이유가 있습니다. 예를 들어 도메인 컨트롤러를 사용할 수 없는 경우 로컬 사용자 계정을 사용하여 SMB 서버에 액세스할 수 있고, 로컬 그룹을 사용하여 권한을 할당하거나 SMB 서버가 작업 그룹에 속해 있는 경우 등이 있습니다. 
---

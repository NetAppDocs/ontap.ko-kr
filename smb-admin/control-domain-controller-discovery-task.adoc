...
summary: ONTAP 9.3부터 도메인 컨트롤러를 검색하는 기본 프로세스를 수정할 수 있습니다. 이를 통해 검색 범위를 사이트 또는 선호하는 DC 풀로 제한할 수 있으며, 환경에 따라 성능 향상을 기대할 수 있습니다. 
---
...
* 로컬 사이트의 DC만 검색됩니다.
+
SMB 서버에 대한  `default-site` 매개변수를 정의하여 사이트 및 서비스에서 사이트에 할당되지 않은 LIF와 함께 이 모드를 사용할 수 있습니다. ONTAP 9.13.1부터는 ONTAP에서 신뢰할 수 있는 도메인에 대한 사이트 정보를 검색할 수 없는 경우 기본 사이트를 대체 사이트로 사용할 수도 있습니다.

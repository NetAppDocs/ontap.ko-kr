...
summary: 다중 도메인 이름 매핑 검색을 사용하면 UNIX 사용자 이름과 Windows 사용자 이름 매핑을 구성할 때 Windows 이름의 도메인 부분에 와일드카드를 사용할 수 있습니다. 이름의 도메인 부분에 와일드카드를 사용하면 ONTAP에서 CIFS 서버의 컴퓨터 계정이 있는 도메인과 양방향 트러스트 관계가 설정된 모든 도메인을 검색할 수 있습니다. 
---

...
summary: 스토리지 가상 머신의 BranchCache 해시 저장소에서 캐시된 모든 해시를 삭제할 수 있습니다. 이는 지점 BranchCache 구성을 변경한 경우 유용할 수 있습니다. 예를 들어, 최근에 캐싱 모드를 분산 캐싱에서 호스팅 캐싱 모드로 재구성한 경우 해시 저장소를 삭제해야 합니다. 
---

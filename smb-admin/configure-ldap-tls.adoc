...
CIFS 서버가 속한 도메인에 Active Directory 인증서 서비스가 이미 설치 및 구성되어 있어야 합니다. Active Directory 인증서 서비스의 설치 및 구성에 대한 정보는 http://technet.microsoft.com/en-us/library/["Microsoft TechNet 라이브러리: technet.microsoft.com"]를 참조하십시오.

[NOTE]
====
ONTAP 9.13.1부터 SMB 서버에 대해  `-use-start-tls-for-ad-ldap` 또는  `-use-ldaps-for-ad-ldap`가 활성화된 경우, 신뢰할 수 있는 도메인과 홈 도메인에 필요한 CA 인증서가 설치되어 있는지 확인하십시오. ONTAP는 신뢰할 수 있는 도메인의 도메인 컨트롤러에 대한 사이트 정보를 확인하기 위해 TLS 또는 LDAPS 연결을 설정할 수 있어야 합니다.
...
[NOTE]
====
ONTAP 9.13.1 버전부터 Active Directory 연결에 대해 보안 LDAP 통신이 활성화된 경우, ONTAP는 신뢰 도메인 컨트롤러에 연결할 때도 해당 보안 연결을 사용합니다. 신뢰 도메인 컨트롤러에 필요한 인증서가 설치되어 있지 않으면 ONTAP가 신뢰 도메인 사이트 정보를 확인할 수 없을 수 있습니다.

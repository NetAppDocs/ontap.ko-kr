...
summary: SMB 서명은 SMB 서버와 클라이언트 간의 네트워크 트래픽이 손상되지 않도록 보호하는 데 도움이 됩니다. 이는 재전송 공격을 방지함으로써 이루어집니다. 기본적으로 ONTAP는 클라이언트 요청 시 SMB 서명을 지원합니다. 선택적으로 스토리지 관리자는 SMB 서버에서 SMB 서명을 필수로 설정하도록 구성할 수 있습니다. 
---

...
summary: Oplock을 사용하면 클라이언트가 파일을 잠그고 콘텐츠를 로컬에 캐시할 수 있으므로 파일 작업의 성능을 향상시킬 수 있습니다. Oplock은 스토리지 가상 머신에 있는 SMB 공유에서 활성화됩니다. 특정 상황에서는 Oplock을 비활성화해야 할 수도 있습니다. 공유별로 Oplock을 활성화 또는 비활성화할 수 있습니다. 
---

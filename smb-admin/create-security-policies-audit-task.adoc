...
summary: 스토리지 가상 머신에 대한 감사 정책을 생성하는 것은 파일 또는 폴더에 ACL을 구성하고 적용하는 세 번째 단계입니다. 정책은 다양한 작업을 담는 컨테이너 역할을 하며, 각 작업은 파일 또는 폴더에 적용할 수 있는 단일 항목입니다. 보안 정책에 작업을 나중에 추가할 수 있습니다. 
---
= ONTAP SMB 보안 정책 생성
:allow-uri-read: 
:icons: font

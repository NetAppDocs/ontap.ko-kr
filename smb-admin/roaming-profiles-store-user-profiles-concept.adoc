...
summary: ONTAP는 스토리지 가상 머신과 연결된 CIFS 서버에 Windows 로밍 프로필을 저장하는 것을 지원합니다. 사용자 로밍 프로필을 구성하면 사용자가 로그인하는 위치에 관계없이 리소스를 자동으로 사용할 수 있는 등의 이점을 누릴 수 있습니다. 또한 로밍 프로필은 사용자 프로필의 관리 및 운영을 간소화합니다. 
---

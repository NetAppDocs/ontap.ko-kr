...
summary: SMB 서버는 Active Directory 환경의 컴퓨터에 적용되는 그룹 정책 속성이라고 하는 규칙 집합인 그룹 정책 개체를 지원합니다. GPO를 사용하면 동일한 Active Directory 도메인에 속한 클러스터의 모든 스토리지 가상 머신에 대한 설정을 중앙에서 관리할 수 있습니다. 
---

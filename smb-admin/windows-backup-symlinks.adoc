...
summary: Windows에서 실행되는 백업 애플리케이션이 Unix 스타일의 심볼 링크를 발견하면 해당 링크를 따라가 데이터를 백업합니다. ONTAP 9.15.1부터는 데이터 대신 심볼 링크를 백업하는 옵션을 사용할 수 있습니다. 이 기능은 ONTAP FlexGroup 볼륨 및 FlexVols에서 완전히 지원됩니다. 
---
...
ONTAP CLI 외에도 ONTAP REST API를 사용하여 이 기능을 관리할 수 있습니다. 자세한 내용은 https://docs.netapp.com/us-en/ontap-automation/whats-new.html["ONTAP REST API 및 자동화의 새로운 기능"^]을 참조하십시오. ONTAP이 Unix 스타일 심볼 링크를 처리하는 방식을 결정하는 구성은 각 SVM에 대해 별도로 수행해야 합니다.

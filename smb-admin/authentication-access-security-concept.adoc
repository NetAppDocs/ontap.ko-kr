...
summary: 사용자가 SVM에 저장된 데이터에 액세스하기 위해 SMB 연결을 생성하려면 먼저 SMB 서버가 속한 도메인에서 인증을 받아야 합니다. SMB 서버는 Kerberos와 NTLM 두 가지 인증 방식을 지원합니다. Kerberos는 도메인 사용자를 인증하는 데 사용되는 기본 방식입니다. 
---

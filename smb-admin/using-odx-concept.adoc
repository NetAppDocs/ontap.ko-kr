...
+
ODX 복사 오프로드 패스스루는 Hyper-V 게스트 운영 체제에서 시작된 복사 작업을 지원합니다. 게스트 운영 체제는 ODX를 지원해야 하며, ODX 지원 SMB 또는 SAN 스토리지를 사용하는 SCSI 가상 디스크를 사용해야 합니다. IDE 가상 디스크는 ODX 패스스루를 지원하지 않습니다. 소스 및 대상 스토리지는 동일한 클러스터에 있어야 합니다.

+
자세한 내용은 link:use-cases-odx-concept.html["ONTAP SMB 서버에서 ODX 사용 사례"]을 참조하십시오.

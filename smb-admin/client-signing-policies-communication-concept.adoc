...
[NOTE]
====
Microsoft는 더 이상  `Digitally sign communications (if client agrees)` 또는  `Digitally sign communications (if server agrees)` 그룹 정책 설정 사용을 권장하지 않습니다. 또한 Microsoft는 더 이상  `EnableSecuritySignature` 레지스트리 설정 사용을 권장하지 않습니다. 이러한 옵션은 SMB 1 동작에만 영향을 미치며  `Digitally sign communications (always)` 그룹 정책 설정 또는  `RequireSecuritySignature` 레지스트리 설정으로 대체할 수 있습니다. link:http://blogs.technet.com/b/josebda/archive/2010/12/01/the-basics-of-smb-signing-covering-both-smb1-and-smb2.aspx["Microsoft 블로그: SMB 서명의 기본 사항(SMB1 및 SMB2 모두 포함)"^]에서 자세한 정보를 확인할 수도 있습니다.

...
summary: 기본적으로 ONTAP는 90분마다 그룹 정책 개체 변경 사항을 검색하고 적용합니다. 보안 설정은 16시간마다 새로 고쳐집니다. ONTAP가 자동으로 업데이트하기 전에 새 GPO 정책 설정을 적용하도록 GPO를 업데이트하려면 ONTAP 명령을 사용하여 CIFS 서버에서 수동 업데이트를 트리거할 수 있습니다. 
---

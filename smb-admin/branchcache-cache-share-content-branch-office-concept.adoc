...
summary: BranchCache는 Microsoft에서 요청하는 클라이언트의 로컬 컴퓨터에 콘텐츠를 캐싱할 수 있도록 개발되었습니다. ONTAP에서 BranchCache를 구현하면 지사 사용자가 SMB를 사용하여 스토리지 가상 머신에 저장된 콘텐츠에 액세스할 때 광역 네트워크 사용량을 줄이고 액세스 응답 시간을 향상시킬 수 있습니다. 
---

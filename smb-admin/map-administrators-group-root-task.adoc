...
summary: 환경에 CIFS 클라이언트만 있고 스토리지 가상 머신이 멀티프로토콜 스토리지 시스템으로 설정된 경우 SVM의 파일에 액세스하려면 루트 권한이 있는 Windows 계정이 하나 이상 있어야 합니다. 그렇지 않으면 사용자 권한이 부족하여 SVM을 관리할 수 없습니다. 
---

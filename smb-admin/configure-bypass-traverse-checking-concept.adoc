...
summary: 우회 통과 검사는 사용자가 통과된 디렉토리에 대한 권한이 없는 경우에도 파일 경로의 모든 디렉토리를 통과할 수 있는지 여부를 결정하는 사용자 권한입니다. 우회 통과 검사를 허용하거나 허용하지 않을 때 발생하는 상황과 스토리지 가상 머신에서 사용자에 대한 우회 통과 검사를 구성하는 방법을 이해해야 합니다. 
---
...
* xref:assign-privileges-concept.html[지원되는 권한 목록입니다]

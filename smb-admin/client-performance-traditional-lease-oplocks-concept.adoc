...
summary: 기존의 oplock과 임대 oplock은 특정 파일 공유 시나리오에서 SMB 클라이언트가 읽기 선행, 쓰기 지연 및 잠금 정보를 클라이언트 측에서 캐싱할 수 있도록 합니다. 이를 통해 클라이언트는 서버에 해당 파일에 대한 액세스가 필요함을 정기적으로 알리지 않고도 파일을 읽거나 쓸 수 있습니다. 이는 네트워크 트래픽을 줄여 성능을 향상시킵니다. 
---

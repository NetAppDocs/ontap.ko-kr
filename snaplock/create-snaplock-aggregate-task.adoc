...
summary: '볼륨 `-snaplock-type` 옵션을 사용하여 규정 준수 또는 엔터프라이즈 SnapLock 볼륨 유형을 지정할 수 있습니다. ONTAP 9.10.1 이전 버전에서는 별도의 SnapLock 애그리게이트를 생성해야 합니다. ONTAP 9.10.1부터는 SnapLock 볼륨과 비 SnapLock 볼륨을 동일한 애그리게이트에 함께 사용할 수 있습니다. 따라서 ONTAP 9.10.1을 사용하는 경우 더 이상 별도의 SnapLock 애그리게이트를 생성할 필요가 없습니다.' 
---

...
summary: SnapLock ComplianceClock은 WORM 파일의 보존 기간을 변경할 수 있는 변조를 방지합니다. SnapLock 애그리게이트를 호스팅하는 각 노드에서 시스템 Compliance Clock을 초기화해야 합니다. 
---
= ONTAP 규정 준수 시계 초기화
:allow-uri-read: 
:icons: font
...
. *규정 준수 시계* 열을 표시하고 규정 준수 시계가 초기화되었는지 확인하려면 *클러스터 > 개요 > 노드* 섹션에서 *표시/숨기기*를 선택하고 *SnapLock 규정 준수 시계*를 선택하십시오.

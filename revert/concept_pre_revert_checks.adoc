...
* link:arp-warning-clear-revert.html["비정상적인 활동에 대한 자율적 랜섬웨어 방어 경고에 대응"].




| ONTAP 9.19.1  a| 
* 액티브-액티브 다중 경로 구성을 사용하는 SVM이 있는 경우, 반드시 link:delete-active-active-svms.html["SVM을 삭제합니다"].
* ONTAP 9.19.1 이상에서 생성된 QoS 정책 그룹의 기본 설정을 수정한 경우, 반드시 link:modify-qos-policy-groups.html["재설정"].
* 볼륨을 ONTAP 9.19.1로 업그레이드하고 사용자에게 두 번째 S3 액세스 키를 추가한 경우 link:second-s3-access-key.html["이 키 제거"]해야 합니다.




| ONTAP 9.18.1  a| 
* ONTAP 9.18.1 업그레이드의 일부로 ARP/AI에 대한 자동 활성화가 설정된 경우 link:anti-ransomware-disable-auto-enable.html["사용하지 않도록 설정합니다"]해야 합니다.




| ONTAP 9.17.1  a| 
* ONTAP SAN에 대해 ONTAP ARP/AI 기능을 활성화한 경우, link:anti-ransomware-disable-san.html["사용하지 않도록 설정합니다"].

...
+
그만큼 `-version` 옵션은 되돌릴 ONTAP 릴리스 버전을 나타냅니다. ONTAP 버전의 경우 처음 두 값만 필수입니다. 예를 들어, 9.14.1에서 9.13.1로 되돌리는 경우, 올바른 값은 다음과 같습니다. `-version` 옵션은 9.13입니다.
...
. 두 노드 모두 원래 상태로 복원된 후 클러스터에 대한 고가용성(HA)을 다시 활성화하십시오.
+
[source, cli]
----
...
NOTE: 클러스터 HA를 다시 활성화하면  `storage failover auto-giveback` 설정이 true로 되돌아갈 수 있습니다.
...
----
. 양쪽 노드의 `storage failover auto-giveback` 설정을 확인하십시오.
+
[source, cli]
----
...
----
+
사용 환경에 따라 다른 설정이 필요한 경우,  `storage failover modify -node <nodename> -auto-giveback <true|false>` 명령을 사용하여 수정하십시오.
...
* link:https://kb.netapp.com/on-prem/ontap/Ontap_OS/OS-KBs/Why_auto-giveback_enabled_after_re-enabling_storage_failover_setting["NetApp 기술 자료: 스토리지 페일오버 설정을 다시 활성화한 후 자동 반환 기능이 활성화되는 이유는 무엇입니까?"^]

...
** 링별 쿼럼 기본 노드는 모든 노드에서 동일해야 합니다.
+
각 링마다 쿼럼 기본 요소가 다를 수 있다는 점에 유의하십시오.
...
----
+
를 참조하십시오link:https://kb.netapp.com/Advice_and_Troubleshooting/Data_Storage_Software/ONTAP_OS/Volume_Showing_WAFL_Inconsistent["NetApp 지식 기반: 볼륨에 WAFL 일관되지 않음 표시"^] 일관되지 않은 볼륨을 해결하는 방법에 대해.

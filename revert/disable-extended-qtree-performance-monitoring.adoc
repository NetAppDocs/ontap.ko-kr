...
당신은  `ext_performance_monitoring.enabled` 설정을  `/api/storage/qtrees` 엔드포인트를 사용하여 작업합니다. 현재 값을 가져오려면 GET을 사용하고, 새 qtree를 생성할 때 설정하려면 POST를 사용하며, 기존 qtree에서 변경하려면 PATCH를 사용합니다.

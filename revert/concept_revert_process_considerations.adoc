...
+
link:https://docs.netapp.com/us-en/ontap-afx/administer/revert-ontap.html["AFX 스토리지 시스템"^]복귀를 지원하지 않습니다.
...
IMPORTANT: 일부 노드만 되돌렸지만 모든 노드를 되돌린 경우 클러스터를 소스 릴리스로 업그레이드하지 마세요.
...
* MetroCluster 호환이지만 MetroCluster 검증을 받지 않은 스위치를 사용하는 MetroCluster IP 시스템에서 ONTAP 9.7에서 9.6으로 복귀하면 ONTAP 9.6 및 이전 버전을 사용하는 시스템에 대한 지원이 없으므로 중단이 발생합니다.
* 노드를 ONTAP 9.13.1 이전 버전으로 되돌리려면 먼저 암호화된 SVM 루트 볼륨을 암호화되지 않은 볼륨으로 변환해야 합니다
+
SVM 루트 볼륨 암호화를 지원하지 않는 ONTAP 버전으로 되돌리려고 하면 시스템은 경고를 표시하고 되돌리기를 차단합니다.

* ONTAP 9.19.1 이상 버전에서 NFS over TLS가 활성화된 경우, 이전 버전의 ONTAP으로 되돌리기 전에 모든 NFS over TLS 구성 인스턴스를 비활성화해야 합니다. NFS over TLS는 ONTAP 9.19.1 버전부터 지원됩니다.

...
지연 시간이 눈에 띄게 증가하는 경우 다음을 참조하세요.link:https://kb.netapp.com/Advice_and_Troubleshooting/Data_Storage_Software/ONTAP_OS/High_or_fluctuating_latency_after_turning_on_NetApp_ONTAP_File_System_Analytics["NetApp 지식 기반: NetApp ONTAP 파일 시스템 분석을 켠 후 대기 시간이 길어지거나 변동함"^] .
...
용량 분석을 활성화하면 ONTAP가 용량 분석 초기화 스캔을 수행합니다. 이 스캔은 용량 분석이 활성화된 볼륨의 모든 파일에 대한 메타데이터에 액세스합니다. 스캔 중에는 파일 데이터를 읽지 않습니다. ONTAP 9.14.1부터는 ONTAP REST API, System Manager의 **Explorer** 탭 또는 `volume analytics show` ONTAP CLI 명령을 통해 스캔 진행 상황을 추적할 수 있습니다. 스로틀링 이벤트가 발생하면 ONTAP가 알림을 제공합니다.

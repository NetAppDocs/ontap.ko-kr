...
[role="lead"]
ONTAP 9.12.1 버전부터 ONTAP에는 사전 정의된 역할 기반 액세스 제어(RBAC) 역할인  `admin-no-fsa`이 포함되어 있습니다.  `admin-no-fsa` 역할은 관리자 수준의 권한을 부여하지만, 사용자가 ONTAP CLI, ONTAP REST API 및 System Manager에서  `files` 엔드포인트(예: 파일 시스템 분석)와 관련된 작업을 수행하는 것을 방지합니다.

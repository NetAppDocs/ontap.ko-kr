...
summary: ONTAP 9.8부터는 IP 보안을 사용하여 네트워크 트래픽을 보호할 수 있습니다. IPsec은 ONTAP에서 사용할 수 있는 여러 데이터 이동 중 또는 전송 중 암호화 옵션 중 하나입니다. 운영 환경에서 사용하기 전에 IPsec 구성을 준비해야 합니다. 
---
= ONTAP 네트워크의 IP 보안에 대해 알아보기
:hardbreaks:
:allow-uri-read: 
:nofooter: 
:icons: font
...
ONTAP 9.16.1 이상 버전을 사용하는 경우, 암호화 및 무결성 검사 등 연산 집약적인 작업을 스토리지 노드에 설치된 네트워크 인터페이스 컨트롤러(NIC) 카드로 오프로드할 수 있습니다. NIC 카드로 오프로드된 IPsec 암호화 작업의 네트워크 처리량 성능 영향은 IPsec 암호화가 활성화되지 않은 평문 작업에 비해 약 5% 이하입니다. 이를 통해 IPsec으로 보호되는 네트워크 트래픽의 성능과 처리량을 크게 향상시킬 수 있습니다.
...
* X50131A (IO 모듈, 2PT, 100/200GbE)
* X50131B (IO 모듈, 2PT, 200/400GbE,B)
* X60132A(4P, 10G/25G 이더넷 컨트롤러)


ONTAP 9.17.1에서는 다음 이더넷 카드에 대한 지원이 추가되었습니다.

* X50135A (IO 모듈, 2PT, 100GbE, 암호화)
* X60135A(2p, 40G/100G 이더넷 컨트롤러)


X50131A, X50131B 및 X50135A 카드는 다음 플랫폼에서 지원됩니다.

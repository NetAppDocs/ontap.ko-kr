...
summary: ONTAP 9.8 이상 버전의 NAS 경로 페일오버 구성. 
---
= ONTAP용 NAS 경로 페일오버 워크플로우
:hardbreaks:
:allow-uri-read: 
:nofooter: 
:icons: font
...
NOTE: SAN LIF는 (링크 장애 발생 후 수동으로 이동하지 않는 한) 마이그레이션되지 않습니다. 대신 호스트의 멀티패싱 기술이 트래픽을 다른 LIF로 전환합니다. 자세한 내용은 link:../san-admin/index.html["SAN 관리"]를 참조하십시오.

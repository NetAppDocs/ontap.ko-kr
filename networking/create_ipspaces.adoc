...
summary: 클러스터의 각 SVM에 대한 IPspace를 생성합니다. 
---
...
[NOTE]
====
ONTAP 9.19.1부터 ONTAP는 PRR 및 RACK이라는 두 가지 손실 복구 기능을 ONTAP TCP 스택에 통합하여 손실이 많고 지연 시간이 긴 WAN 링크에서 TCP 데이터 전송 성능을 향상시킵니다. link:improve-tcp-wan-performance.html["PRR 및 RACK을 사용하여 TCP WAN 성능을 향상시키는 방법에 대해 알아보십시오."]

...
summary: 데이터 센터에서는 네이버 디스커버리 프로토콜을 사용하여 물리적 또는 가상 시스템 쌍과 해당 네트워크 인터페이스 간의 네트워크 연결을 확인할 수 있습니다. ONTAP는 Cisco Discovery Protocol과 Link Layer Discovery Protocol이라는 두 가지 네이버 디스커버리 프로토콜을 지원합니다. 
---
...
인접 장치에 대한 정보를 표시하려면 모든 이더넷 스위치와 라우터에서 CDP를 활성화해야 합니다.
...
NOTE: 노드에서 LIF가 변경될 때 수신 장치(예: 이더넷 스위치)에서 CDP 정보가 업데이트되지 않는 경우가 있습니다. 이러한 문제가 발생하면 노드의 네트워크 인터페이스를 다운 상태로 설정한 다음 업 상태로 설정해야 합니다.
...
여러 공급업체 스위치는 CDP를 지원하지 않습니다. 를 참조하십시오link:https://kb.netapp.com/onprem/ontap/da/NAS/ONTAP_device_discovery_shows_nodes_instead_of_the_switch["NetApp 지식 기반: ONTAP 장치 검색 시 스위치 대신 노드가 표시됨"^] 자세한 내용은.

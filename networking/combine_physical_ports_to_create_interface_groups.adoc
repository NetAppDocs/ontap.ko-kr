...
summary: 두 개 이상의 물리적 포트를 하나의 논리적 포트로 결합하여 인터페이스 그룹을 생성합니다. 
---
...
image:ontap_nm_image6.png["단일 모드 인터페이스 그룹입니다"]
...
image:ontap_nm_image7.png["다이나믹 멀티모드 인터페이스 그룹"]
...
IP 주소, MAC 주소, 순차 또는 포트 기반 로드 밸런싱 방법을 사용하여 다중 모드 인터페이스 그룹의 네트워크 포트를 통해 네트워크 트래픽을 균등하게 분산함으로써 나가는 트래픽에 다중 모드 인터페이스 그룹의 모든 인터페이스가 동일하게 사용되도록 할 수 있습니다.
...
에 대한 자세한 내용은 `network port ifgrp create` link:https://docs.netapp.com/us-en/ontap-cli/network-port-ifgrp-create.html["ONTAP 명령 참조입니다"^]을 참조하십시오.
...
에서 포트 인터페이스 그룹에 적용되는 및 구성 제한에 대해 자세히 `network port ifgrp add-port`link:https://docs.netapp.com/us-en/ontap-cli/network-port-ifgrp-add-port.html["ONTAP 명령 참조입니다"^] 알아보십시오.
...
에 대한 자세한 내용은 `network port ifgrp remove-port` link:https://docs.netapp.com/us-en/ontap-cli/network-port-ifgrp-remove-port.html["ONTAP 명령 참조입니다"^]을 참조하십시오.
...
에 대한 자세한 내용은 `network port ifgrp delete` link:https://docs.netapp.com/us-en/ontap-cli/network-port-ifgrp-delete.html["ONTAP 명령 참조입니다"^]을 참조하십시오.

...
summary: NFS 또는 SMB 서버를 생성하기 전에 SVM에 대한 DNS 서비스를 구성합니다. 
---
...
NOTE: DDNS 업데이트에 대한 RFC 규칙을 준수하지 않는 SVM FQDN의 구성 오류를 방지하려면 RFC를 준수하는 FQDN 이름을 사용하십시오. 자세한 내용은 link:https://tools.ietf.org/html/rfc1123["RFC 1123"^]를 참조하십시오.

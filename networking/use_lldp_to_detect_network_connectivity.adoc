...
인접 장치에 대한 정보를 표시하려면 모든 이더넷 스위치와 라우터에서 LLDP를 활성화해야 합니다.
...
* 클러스터와 이더넷 스위치 간에 표시되는 정보가 다를 수 있습니다.
+
이더넷 스위치에 표시되는 섀시 ID 및 포트 ID 데이터는 CNA 포트와 비 CNA 포트에서 서로 다를 수 있습니다.

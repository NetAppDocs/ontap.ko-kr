...
summary: 클러스터의 모든 활성 또는 수신 대기 연결을 보거나 클라이언트, LIF, 프로토콜 또는 서비스별로 노드의 활성 연결 수를 확인합니다. 
---
= ONTAP 클러스터 연결을 봅니다
:hardbreaks:
:allow-uri-read: 
...
network connections active show-protocols 명령을 사용하여 노드의 프로토콜별 활성 연결 수를 표시합니다.
 ...
 `network connections active show-protocols`에 대한 자세한 내용은 link:https://docs.netapp.com/us-en/ontap-cli/search.html?q=network+connections+active+show-protocols["ONTAP 명령 참조입니다"^]을 참조하십시오.

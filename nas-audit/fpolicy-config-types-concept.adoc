...
summary: 두 가지 기본 FPolicy 구성 유형이 있습니다. 한 구성은 외부 FPolicy 서버를 사용하여 알림을 처리하고 조치를 취합니다. 다른 구성은 외부 FPolicy 서버를 사용하지 않습니다. 대신 확장자를 기반으로 한 간단한 파일 차단을 위해 ONTAP 내부 네이티브 FPolicy 서버를 사용합니다. 
---
= ONTAP FPolicy 구성 유형
:allow-uri-read: 
:icons: font

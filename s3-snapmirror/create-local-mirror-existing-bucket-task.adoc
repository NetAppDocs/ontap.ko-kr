...
summary: 예를 들어 ONTAP 9.10.1 이전 릴리스에서 S3 구성을 업그레이드한 경우와 같이 언제든지 동일한 클러스터에 있는 기존 S3 버킷을 보호할 수 있습니다. 데이터를 다른 스토리지 VM에 있는 버킷 또는 소스와 동일한 스토리지 VM에 있는 버킷으로 미러링할 수 있습니다. 
---

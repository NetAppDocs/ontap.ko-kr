...
summary: '기본적으로 ONTAP는 NFS Kerberos에 대해 DES, 3DES, AES-128 및 AES-256 암호화 유형을 지원합니다. `vserver nfs modify` 명령에 `-permitted-enc-types` 매개변수를 사용하여 각 SVM에 허용되는 암호화 유형을 구성하여 특정 환경의 보안 요구 사항을 충족할 수 있습니다.' 
---

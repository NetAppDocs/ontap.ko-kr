...
NOTE: `multi-admin-verify`종료 상태의 요청은 자동으로 덮어쓰이거나 제거될 수 있습니다.  사용하세요link:../system-admin/audit-display-log-contents-task.html["감사 로그"] 이전 요청을 검토합니다.
...
.시스템 관리자
--
MAV 관리자는 승인 요청 세부 정보, 요청 만료 기간, 요청을 승인하거나 거부할 수 있는 링크가 포함된 이메일 메시지를 받습니다.  이메일의 링크를 클릭하여 승인 대화 상자에 접근하거나 시스템 관리자에서 *이벤트 및 작업 > 요청*으로 이동할 수 있습니다.

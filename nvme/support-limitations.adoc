...
| 9.17.1  a| 
* VMware 워크로드를 위한 SnapMirror 액티브 싱크 NVMe/FC 및 NVMe/TCP 호스트 액세스.
...
* 프로토콜 공존
+
SCSI, NAS 및 NVMe 프로토콜은 동일한 스토리지 가상 머신(SVM)에 존재할 수 있습니다. 호스트 운영 체제에서 프로토콜 공존을 지원하지 않는 경우 호스트에서 지원하는 각 프로토콜에 대해 별도의 SVM을 구성해야 합니다.

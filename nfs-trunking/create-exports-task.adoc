...
* SVM 루트 볼륨의 기본 엑스포트 정책에는 모든 클라이언트가 NFS를 통해 자유롭게 액세스할 수 있도록 허용하는 규칙이 포함되어야 합니다.
+
link:../nfs-config/open-export-policy-svm-root-volume-task.html["ONTAP SVM에서 NFS 클라이언트 액세스."]에 대해 자세히 알아보십시오

* 사용자 지정 엑스포트 정책을 생성한 경우 기본 정책에도 동일한 규칙을 추가해야 합니다.
+
link:../nfs-admin/default-export-policy-svms-concept.html["ONTAP NFS SVM의 기본 엑스포트 정책"]에 대해 자세히 알아보십시오

...
NOTE: FabricPool 계층화가 활성화된 원본 볼륨에 대한 캐시로 FlexCache 볼륨을 생성할 수 있지만 FlexCache 볼륨 자체는 계층화할 수 없습니다.
...
ONTAP 9.5 버전부터 지원됩니다. SVM DR 관계의 기본 SVM은 원본 볼륨을 사용할 수 있습니다. 그러나 ONTAP ONTAP 이전 버전을 실행 중인 경우 SVM DR 관계가 끊어지면 새 원본 볼륨을 사용하여 FlexCache 관계를 다시 생성해야 합니다.

ONTAP 9.18.1 버전부터 원본 SVM이 장애 조치를 수행하면 캐시가 자동으로 재해 복구 사이트의 원본으로 전환됩니다. 수동 복구 단계가 필요 없어집니다.

xref:create-volume-task.adoc[FlexCache 볼륨 생성 방법에 대해 알아보세요.]
 a| 
아니요

FlexCache 원본 볼륨은 기본 SVM에는 생성할 수 있지만 보조 SVM에는 생성할 수 없습니다. FlexCache 관계는 복제되지 않지만 원본 볼륨 데이터는 일반 볼륨처럼 복제됩니다.
...
NOTE: ONTAP 9.5 이전 ONTAP 9 릴리스에서는 원본 FlexVol 볼륨이 7-Mode로 작동하는 Data ONTAP 8.2.x 시스템에서 생성된 FlexCache 볼륨에만 데이터를 제공할 수 있었습니다. ONTAP 9.5부터는 원본 FlexVol 볼륨이 ONTAP 9 시스템의 FlexCache 볼륨에도 데이터를 제공할 수 있습니다. 7-Mode FlexCache에서 ONTAP 9 FlexCache로 마이그레이션하는 방법에 대한 자세한 내용은 link:https://www.netapp.com/pdf.html?item=/media/7336-tr4743.pdf["NetApp 기술 보고서 4743: FlexCache in ONTAP"^]을 참조하십시오.

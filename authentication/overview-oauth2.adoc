...
summary: 'ONTAP 9.14 버전부터는 Open Authorization(OAuth 2.0) 프레임워크를 사용하여 ONTAP 클러스터에 대한 액세스를 제어할 수 있습니다. 이 기능은 ONTAP CLI, System Manager, ONTAP REST API를 포함한 모든 ONTAP 관리 인터페이스를 통해 구성할 수 있습니다. 단, OAuth 2.0 인증 및 액세스 제어 결정은 클라이언트가 ONTAP REST API를 사용하여 ONTAP에 액세스할 때만 적용됩니다.' 
---
...
[role="lead"]
ONTAP 9.14 버전부터는 Open Authorization(OAuth 2.0) 프레임워크를 사용하여 ONTAP 클러스터에 대한 액세스를 제어할 수 있습니다. 이 기능은 ONTAP CLI, System Manager, ONTAP REST API를 포함한 모든 ONTAP 관리 인터페이스를 통해 구성할 수 있습니다. 단, OAuth 2.0 인증 및 액세스 제어 결정은 클라이언트가 ONTAP REST API를 사용하여 ONTAP에 액세스할 때만 적용됩니다.

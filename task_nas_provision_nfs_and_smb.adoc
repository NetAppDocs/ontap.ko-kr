...
FlexVol 볼륨을 만들거나, 높은 성능 요구 사항이 있는 대규모 파일 시스템의 경우 FlexGroup 볼륨을 만들 수 있습니다. 보다 link:../flexgroup/provision-nas-flexgroup-task.html["FlexGroup 볼륨을 사용하여 대용량 파일 시스템용 NAS 스토리지를 프로비저닝합니다"] .

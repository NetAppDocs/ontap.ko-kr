...
[role="lead"]
_SnapMirror cloud_ 는 데이터 보호 워크플로우를 클라우드 또는 온프레미스 객체 스토리지 솔루션으로 전환하려는 ONTAP 사용자를 위해 설계된 백업 및 복구 기술입니다. 기존의 테이프 백업 아키텍처에서 벗어나려는 조직은 객체 스토리지를 장기 데이터 보존 및 아카이빙을 위한 대체 저장소로 활용할 수 있습니다. SnapMirror 클라우드는 증분 방식의 영구 백업 전략의 일환으로 ONTAP에서 객체 스토리지로의 복제를 제공합니다.

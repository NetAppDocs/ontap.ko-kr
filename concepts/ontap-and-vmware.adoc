...
---
= ONTAP 및 VMware vSphere에 대해 알아보기
:hardbreaks:
:allow-uri-read: 
:nofooter: 
:icons: font
...
NOTE: ONTAP 9.15.1 버전부터 SCSI-3을 사용하여 가상 볼륨에 대한 영구 예약을 생성할 수 있습니다. 이 기능은 Windows Server 장애 조치 클러스터(WSFC)가 포함된 ONTAP tools for VMware vSphere 9를 사용할 때만 지원됩니다.
...
VMware Live Site Recovery(이전 명칭: Site Recovery Manager(SRM))는 재해 복구 기능을 제공합니다. Live Site Recovery는 ONTAP tools for VMware vSphere와 통합되어 ONTAP 데이터 관리 기능에 액세스하고 활용합니다.

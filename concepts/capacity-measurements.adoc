...
summary: 'ONTAP 스토리지 용량은 물리적 공간 또는 논리적 공간으로 측정할 수 있습니다. 이러한 측정값의 표시 방식은 ONTAP 버전과 System Manager 또는 Command Line Interface(CLI) 사용 여부에 따라 다릅니다. 스토리지 사용량을 모니터링할 때는 용량 관련 용어, 물리적 용량과 논리적 용량의 차이점, 그리고 용량 유형이 표시되는 방식을 이해하는 것이 중요합니다.' 
---
= ONTAP 용량 보고 및 측정에 대해 알아보세요
:hardbreaks:
:toclevels: 1
:allow-uri-read: 
:toclevels: 1
...
[role="lead"]
ONTAP 스토리지 용량은 물리적 공간과 논리적 공간으로 측정할 수 있습니다. 이러한 측정값의 표시 방식은 ONTAP 버전과 System Manager 또는 Command Line Interface(CLI) 사용 여부에 따라 다릅니다. 스토리지 사용량을 모니터링할 때는 용량 관련 용어, 물리적 용량과 논리적 용량의 차이점, 그리고 용량 유형이 표시되는 방식을 이해하는 것이 중요합니다.
...
ONTAP CLI에서 물리적으로 사용된 공간은 ONTAP 버전에 따라 다르게 보고됩니다. ONTAP 9.13.1 및 이전 버전에서는  `physical used` 매개변수가  `volume show` 명령어 출력에 표시되며, 볼륨에 적용된 모든 스토리지 효율성 절감 효과를 포함합니다. ONTAP 9.14.1부터는  `physical used` 매개변수가 온도 민감 스토리지(TSSE), 인라인 압축, 컴팩션, 볼륨 간 중복제거와 같은 애그리게이트 수준의 스토리지 효율성 절감 효과를 포함하지 않으므로, 볼륨의 실제 물리적 사용 공간을 정확하게 반영하지 않습니다. ONTAP 9.14.1 및 이후 버전에서는 볼륨의 정확한 물리적 사용 공간을 확인하려면  `vol show-footprint` 명령어와  `effective_total_footprint` 매개변수를 사용해야 합니다.

[cols="4"]
|===
| ONTAP 버전 | 명령 | 매개 변수 | 설명 


.3+| 9.14.1 이상 | 볼륨 쇼 풋프린트 | `effective_total_footprint` | 애그리게이트의 절감 효과를 포함하여 실제 사용된 디스크 공간을 반영합니다. 


| `volume show-space` | `physical used` | 애그리게이트의 절감 효과를 포함하여 볼륨 레벨 스토리지 효율성 절감 효과를 반영합니다. 


| '볼륨 쇼' | `physical used` | 집계 수준의 스토리지 효율성 절감 효과가 포함되지 않으므로 link:../volumes/enable-temperature-sensitive-efficiency-concept.html["TSSE 지원 플랫폼"]의 볼륨 물리적 사용 공간을 정확하게 반영하지 않습니다. 


.3+| 9.13.1 이하 | 볼륨 쇼 풋프린트 | `effective_total_footprint` | 애그리게이트의 절감 효과를 포함하여 실제 사용된 디스크 공간을 반영합니다. 


| `volume show-space` | `physical used` | 볼륨 레벨 스토리지 효율성 절감 효과만 반영합니다. 


| '볼륨 쇼' | `total physical used` | 볼륨 레벨 스토리지 효율성 절감 효과만 반영합니다. 
|===
.관련 정보
* link:https://kb.netapp.com/on-prem/ontap/OHW/OHW-KBs/What_is_the_Change_in_Physical_Used_Space_Reporting_for_9.14.1_and_later["9.14.1 이상에서 물리적 사용 공간 보고의 변경 사항"^]에 대해 자세히 알아보십시오.
* link:../volumes/commands-display-space-usage-reference.html["공간 사용 정보를 표시하는 명령"]에 대해 자세히 알아보십시오.




[[capacity-reporting-in-system-manager]]
== System Manager의 용량 보고

시스템 관리자에서 사용 용량 측정값은 ONTAP 버전에 따라 다르게 표시됩니다. ONTAP 9.7부터 시스템 관리자는 물리적 용량과 논리적 용량 모두를 측정하여 제공합니다.
...
|===
.관련 정보
link:../task_admin_monitor_capacity_in_sm.html["System Manager에서 클러스터, 계층, SVM 용량 모니터링"]

...
---
= ONTAP 클라이언트 인증 및 권한 부여에 대해 알아보기
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
ONTAP는 원격 사용자 인증을 위해 SAML(Security Assertion Markup Language)을 지원합니다. 여러 인기 있는 ID 공급자(IdPs)가 지원됩니다. 지원되는 IdPs 및 SAML 인증 활성화 방법에 대한 자세한 내용은 link:../system-admin/configure-saml-authentication-task.html["SAML 인증을 구성합니다"]을 참조하십시오.
...
ONTAP 9.14 버전부터 OAuth 2.0(Open Authorization) 프레임워크를 지원합니다. OAuth 2.0을 사용하여 권한 부여 및 액세스 제어 결정을 내리려면 클라이언트가 ONTAP REST API를 통해 ONTAP에 액세스해야 합니다. 하지만 ONTAP CLI, System Manager, ONTAP REST API 등 모든 ONTAP 관리 인터페이스에서 이 기능을 구성하고 활성화할 수 있습니다.

...
summary: 많은 NetApp 고객은 거의 액세스하지 않는 데이터를 상당량 보유하고 있습니다. 이를 콜드 데이터라고 합니다. 또한 자주 액세스하는 데이터도 있는데, 이를 핫 데이터라고 합니다. 이상적으로는 최상의 성능을 위해 핫 데이터는 가장 빠른 스토리지에 보관하는 것이 좋습니다. 콜드 데이터는 필요할 때 즉시 액세스할 수 있다면 속도가 느린 스토리지로 이동할 수 있습니다. 어떤 데이터가 핫 데이터이고 어떤 데이터가 콜드 데이터인지 파악하는 방법을 알아보세요. 
---
= FabricPool
:allow-uri-read: 
:icons: font
...
.관련 정보
link:../concept_cloud_overview.html["FabricPool 시스템 관리자 문서"]

https://docs.netapp.com/us-en/data-services-cloud-tiering/index.html["NetApp 클라우드 계층화"^]

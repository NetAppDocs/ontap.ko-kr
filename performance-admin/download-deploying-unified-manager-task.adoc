...
summary: 소프트웨어를 설치하려면 가상 어플라이언스 설치 파일을 다운로드한 다음 VMware vSphere Client를 사용하여 해당 파일을 VMware ESXi 서버에 배포해야 합니다. VA는 OVA 파일로 제공됩니다. 
---
= ONTAP Active IQ Unified Manager 다운로드하고 배포하세요.
:allow-uri-read: 
:icons: font
:imagesdir: ../media/

...
summary: 라이선스는 하나 이상의 소프트웨어 사용 권한에 대한 기록입니다. ONTAP 9.10.1부터 모든 라이선스는 NetApp 라이선스 파일 형태로 제공되며, 이 파일은 여러 기능을 사용할 수 있도록 하는 단일 파일입니다. 2023년 5월부터 모든 AFF 시스템과 FAS 시스템은 ONTAP One 소프트웨어 제품군 또는 ONTAP Base 소프트웨어 제품군과 함께 판매되며, 2023년 6월부터 모든 ASA 시스템은 ONTAP One for SAN과 함께 판매됩니다. 
---
= ONTAP 라이센스 개요
:allow-uri-read: 
:icons: font
...
ONTAP One에는 사용 가능한 모든 라이선스 기능이 포함되어 있습니다. 이는 기존의 Core 번들, Data Protection 번들, Security and Compliance 번들, Hybrid Cloud 번들, Encryption 번들의 콘텐츠를 모두 통합한 것입니다. 암호화 기능은 제한된 국가에서는 사용할 수 없습니다.
...
2023년 6월부터 28자리 라이선스 키를 사용하는 ONTAP 시스템도 link:https://kb.netapp.com/onprem/ontap/os/How_to_get_an_ONTAP_One_license_when_the_system_has_28_character_keys["ONTAP One 또는 ONTAP 기본 호환성 번들로 업그레이드하십시오"^]할 수 있습니다.

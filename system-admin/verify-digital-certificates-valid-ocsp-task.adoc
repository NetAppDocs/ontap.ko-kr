...
summary: 'OCSP(Online Certificate Status Protocol)를 사용하면 TLS(전송 계층 보안) 통신을 사용하는 ONTAP 애플리케이션이 OCSP가 활성화된 경우 디지털 인증서 상태를 수신할 수 있습니다. 특정 애플리케이션에 대해 OCSP 인증서 상태 확인을 언제든지 활성화 또는 비활성화할 수 있습니다. 기본적으로 OCSP 인증서 상태 확인은 비활성화되어 있습니다.' 
---

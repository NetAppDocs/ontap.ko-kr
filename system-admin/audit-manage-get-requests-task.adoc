...
summary: 기본적으로 SET 요청은 로그에 기록되지만 GET 요청은 기록되지 않습니다. 하지만 settings 명령을 사용하여 ONTAP HTML, ONTAP CLI 또는 ONTAP API에서 전송된 get 요청을 파일에 기록할지 여부를 제어할 수 있습니다. 
---
...
[role="lead"]
SET 요청은 기본적으로 로그에 기록되며 비활성화할 수 없지만, GET 요청은 기본적으로 로그에 기록되지 않습니다. 그러나 ONTAP HTML(`-httpget`, ONTAP CLI(`-cliget` 또는 ONTAP API(`-ontapiget`에서 전송된 GET 요청을 파일에 기록할지 여부를 제어할 수 있습니다.

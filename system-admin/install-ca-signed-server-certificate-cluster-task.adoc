...
summary: SSL 서버가 클러스터 또는 스토리지 가상 머신을 SSL 클라이언트로 인증할 수 있도록 하려면 클러스터 또는 SVM에 클라이언트 유형의 디지털 인증서를 설치합니다. 그런 다음 client-ca 인증서를 SSL 서버 관리자에게 제공하여 서버에 설치하도록 합니다. 
---
= ONTAP 클러스터에 대한 CA 서명 서버 인증서 설치
:allow-uri-read: 
:icons: font
:imagesdir: ../media/

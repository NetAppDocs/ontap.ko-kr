...
summary: ONTAP 9.3부터 웹 서비스에 대한 Security Assertion Markup Language 인증을 구성할 수 있습니다. 
---
...
NOTE: 일부 URL에는 물음표(?) 문자가 포함되어 있습니다. 물음표는 ONTAP 명령줄 활성 도움말을 활성화합니다. 물음표가 포함된 URL을 입력하려면 먼저 명령 `set -active-help false`을 사용하여 활성 도움말을 비활성화해야 합니다. 활성 도움말은 나중에 명령 `set -active-help true`을 사용하여 다시 활성화할 수 있습니다. 자세한 내용은 link:https://docs.netapp.com/us-en/ontap-cli/set.html["ONTAP 명령 참조입니다"^]를 참조하십시오.

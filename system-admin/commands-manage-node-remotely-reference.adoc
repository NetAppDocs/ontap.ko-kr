...
SP 및 BMC에서 사용할 수 있는 `help` 명령 옵션에 대해 자세히 알아보려면 link:online-help-at-sp-bmc-cli-task.html["ONTAP SP/BMC CLI에서 도움말 정보를 봅니다"]를 참조하십시오.

link:bmc-cli-commands-reference.html["ONTAP 베이스보드 관리 컨트롤러에 대해 지원되는 CLI 명령"]에서 해당 ONTAP BMC CLI에 대해 자세히 알아보십시오.

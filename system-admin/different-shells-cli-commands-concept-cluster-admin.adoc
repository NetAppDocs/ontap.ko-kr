...
클러스터 셸에서 지원되는 Nodeshell 옵션은 다음을 사용하여 액세스할 수 있습니다. `vserver options clustershell` 명령. 이러한 옵션을 보려면 다음을 사용하여 클러스터셸 CLI를 쿼리할 수 있습니다. `vserver options -vserver _nodename_or_clustername_ -option-name ?`

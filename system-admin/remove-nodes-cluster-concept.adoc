...
NOTE: HA 쌍의 첫 번째 노드에서 이 단계를 수행하는 경우 해당 노드만 부적격으로 표시해야 합니다. HA 파트너의 상태를 수정하지 마세요. 파트너 노드가 새로운 마스터로 선택된 경우, 부적격으로 만들기 전에 해당 노드가 엡실론을 보유하고 있는지 확인해야 합니다. 이 작업은 파트너 노드를 제거하기 위해 이 단계를 반복하면 수행됩니다.

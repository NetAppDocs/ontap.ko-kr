...
* ONTAP 9.13.1부터 SSH 인증 프로세스에 인증서 유효성 검사를 선택적으로 추가하여 로그인 보안을 강화할 수 있습니다. 이렇게 하려면 계정이 사용하는 link:../authentication/manage-ssh-public-keys-and-certificates.html["X.509 인증서를 공개 키와 연결합니다"]를 확인합니다. SSH 공개 키와 X.509 인증서를 모두 사용하여 SSH로 로그인하는 경우 ONTAP는 SSH 공개 키로 인증하기 전에 X.509 인증서의 유효성을 확인합니다. 해당 인증서가 만료되었거나 해지된 경우 SSH 로그인이 거부되고 SSH 공개 키는 자동으로 비활성화됩니다.
* ONTAP 9.14.1부터 ONTAP 관리자는 link:../authentication/configure-cisco-duo-mfa-task.html["SSH 인증 프로세스에 Cisco Duo 2단계 인증을 추가합니다"]하여 로그인 보안을 강화할 수 있습니다. Cisco Duo 인증을 활성화한 후 처음 로그인할 때 사용자는 SSH 세션의 인증자 역할을 할 디바이스를 등록해야 합니다.
* ONTAP 9.15.1부터 관리자는 link:../authentication/dynamic-authorization-overview.html["동적 권한 부여를 구성합니다"] 사용자의 신뢰도 점수를 기반으로 SSH 사용자에게 추가 적응형 인증을 제공할 수 있습니다.
* ONTAP 9.19.1 버전부터 관리자는 link:https://docs.netapp.com/us-en/ontap-cli/search.html?q=security+ssh+publickey+show["security ssh publickey show -vserver  -host-key-algorithm  명령"^]을 사용하여 ONTAP 호스트 키 지문을 볼 수 있습니다. 이를 통해 관리자는 해당 지문이 ONTAP에 저장된 지문과 일치하는지 확인할 수 있습니다.

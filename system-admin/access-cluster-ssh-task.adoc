...
* ONTAP 9.19.1 버전부터 관리자는 link:https://docs.netapp.com/us-en/ontap-cli/security-ssh-publickey-show.html["security ssh publickey show -vserver  -host-key-algorithm  명령"^]을 사용하여 ONTAP 호스트 키 지문을 볼 수 있습니다. 이를 통해 관리자는 해당 지문이 ONTAP에 저장된 지문과 일치하는지 확인할 수 있습니다.

...
.단계
기존 NetApp 라이선스 파일 번들이 있는 시스템과 ONTAP 9.10.1 이상을 실행하는 시스템에서 NetApp 라이선스 파일로 변환된 28자 라이선스 키가 있는 시스템에 대해 ONTAP One 라이선스 파일을 다운로드할 수 있습니다. 또한 유료로 ONTAP Base에서 ONTAP One으로 시스템을 업그레이드할 수 있습니다. NetApp 라이선스 파일 다운로드에 대한 자세한 내용은 KB 문서 link:https://kb.netapp.com/nss/Licensing/How_to_download_NLF_licenses_from_the_NetApp_Support_Site["NetApp Support Site에서 NLF 라이센스를 다운로드하는 방법"^]를 참조하십시오.

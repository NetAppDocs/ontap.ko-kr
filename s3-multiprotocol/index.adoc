...
+
기존 데이터가 기존 NAS 클라이언트(NFS 또는 SMB)를 사용하여 생성되었고 NAS 볼륨(FlexVol 또는 FlexGroup 볼륨)에 있는 경우 S3 클라이언트의 분석 도구를 사용하여 이 데이터에 액세스할 수 있습니다.

. NAS 및 S3 프로토콜을 모두 사용하여 I/O를 수행할 수 있는 최신 클라이언트를 위한 백엔드 스토리지
+
NAS와 S3 프로토콜을 모두 사용하여 동일한 데이터를 읽고 쓸 수 있는 Spark 및 Kafka와 같은 애플리케이션에 통합 액세스를 제공할 수 있습니다.
...
ONTAP 멀티프로토콜 지원을 통해 동일한 데이터 세트를 파일 계층 구조나 버킷의 객체로 표현할 수 있습니다. 이를 위해 ONTAP S3 클라이언트가 S3 객체 요청을 사용하여 NAS 스토리지에서 파일을 생성, 읽기, 삭제 및 열거할 수 있도록 하는 "S3 NAS 버킷"을 생성합니다. 이 매핑은 NAS 보안 구성을 준수하며, 파일 및 디렉터리 액세스 권한을 관찰하고 필요에 따라 보안 감사 추적에 기록합니다.
...
S3 NAS "버킷"은 단순히 S3 클라이언트를 위한 NAS 데이터의 매핑일 뿐이며, 표준 S3 버킷이 아닙니다. 따라서 NetApp SnapMirror S3 기능을 사용하여 S3 NAS 버킷을 보호할 필요가 없습니다. 대신 SnapMirror 비동기 볼륨 복제를 사용하여 S3 NAS 버킷이 포함된 볼륨을 보호할 수 있습니다. SnapMirror 동기 및 SVM 재해 복구는 지원되지 않습니다.
...
NOTE: S3 NAS 버킷의 멀티파트 객체는 부분 크기가 아닌 정수 크기로 정렬되어야 합니다. 예를 들어, 파트는 4MB 또는 4GB 또는 이와 유사한 크기일 수 있습니다. 파트는 4.5MB 또는 4000.5MB와 같이 부분 크기 또는 MB 미만의 크기를 사용할 수 없습니다.

멀티파트 업로드는 다음 S3 작업을 지원합니다.

* AbortMultipartUpload 를 클릭합니다
* CompleteMultipartUpload를 클릭합니다
* CopyObject( ONTAP 9.17.1부터)
* CreateMultptUpload 를 클릭합니다
+
ONTAP 9.17.1부터 CreateMultipartUpload는 태그 지정과 사용자 메타데이터 키/값 쌍을 지원합니다.
...
* CopyObject는 SnapLock 볼륨에서 지원되지 않습니다
...
+
--
* ONTAP 9.17.1부터 다중 파트 객체가 포함된 메타데이터가 지원됩니다.
* ONTAP 9.16.1 버전부터 단일 파트 객체의 메타데이터를 지원합니다.
...
+
--
* ONTAP 9.17.1부터 다중 파트 객체가 있는 태그가 지원됩니다.
* ONTAP 9.16.1 버전부터 단일 파트 객체를 포함하는 태그를 지원합니다.

...
+
.. *스토리지 > 스토리지 VM*을 클릭하고 NAS 지원 스토리지 VM을 선택한 다음 *설정*을 클릭하고 *프로토콜 > S3* 아래에서 S3 아래의 image:icon_gear.gif["작업 아이콘"]을 클릭합니다.
...
. NAS 프로토콜이 구성된 스토리지 VM에서 S3를 설정합니다.
+
.. *클러스터 > 스토리지 VM*을 클릭하고 NAS 지원 스토리지 VM을 선택한 다음 *설정*을 클릭하고 *프로토콜 > S3* 아래에서 image:icon_gear.gif["작업 아이콘"]을 클릭합니다.
.. 인증서 유형을 선택합니다. 시스템에서 생성한 인증서 또는 사용자 인증서 중 하나를 선택하든 클라이언트 액세스에 필요합니다.
.. 네트워크 인터페이스를 입력합니다.
...
+
** 비밀 키는 다시 표시되지 않습니다.
** 인증서 정보를 다시 확인해야 하는 경우 *클러스터 > 스토리지 VM*을 클릭하고 스토리지 VM을 선택한 다음 *설정*을 클릭합니다.

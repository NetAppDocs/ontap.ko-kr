...
NOTE: 사용 중 `-quick-resync` 전송된 데이터 블록의 저장 효율성이 제거되어 재동기화 대상 볼륨에서 추가 공간을 소모할 수 있습니다. 이러한 추가 공간 소비는 재동기화 대상에서 인라인 또는 복제 후 스토리지 효율성 적용의 일부로 복구됩니다.
...
+
더 많은 정보를 원하시면 `-quick-resync`, 참조 link:https://docs.netapp.com/us-en/ontap-cli/snapmirror-resync.html["ONTAP 명령 참조입니다"^].
...
====
.관련 정보
* link:resync-svm-destination-task.html["ONTAP SnapMirror 대상 SVM에서 데이터를 다시 동기화합니다"]

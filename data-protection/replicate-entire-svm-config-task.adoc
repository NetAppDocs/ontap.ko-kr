...
* ONTAP 9.19.1 버전부터 SVM DR로 구성된 SVM에서는 IPsec 활성화가 지원되지 않습니다. IPsec을 사용하는 경우, IPsec을 비활성화하거나 다른 SVM을 선택하여 SVM DR 관계를 구성해야 합니다. ONTAP 9.19.1 이상 버전을 사용하는 경우, SVM DR로 구성된 SVM에서 IPsec을 활성화하려고 하면 오류가 발생합니다.
...
. 소스 클러스터에서  `vserver peer create` 명령을 사용하여 SVM 피어 관계를 생성합니다.
+

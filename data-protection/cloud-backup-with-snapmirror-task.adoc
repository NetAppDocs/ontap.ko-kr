...
[role="lead"]
ONTAP 9.9.1부터 데이터 보호 워크플로를 클라우드로 전환할 때 _SnapMirror cloud_를 사용할 수 있습니다. 기존의 테이프 백업 아키텍처 대신 객체 스토리지를 장기 데이터 보존 및 아카이빙을 위한 대체 저장소로 사용할 수 있습니다. 클라우드 객체 스토어로 Amazon S3, Azure Blob, Google Cloud Storage, StorageGRID 또는 ONTAP S3와 같은 공급업체를 사용할 수 있습니다.
...
+
** 클러스터당 6개 이상의 버킷을 사용할 경우, AWS S3, Alibaba Cloud, StorageGrid, Google Cloud Storage, IBM Cloud object storage, ONTAP S3 및 기타 S3 호환 공급자를 포함하여 이를 지원하는 공급자에서는 virtual-hosted-style URL 구성을 사용하는 것이 좋습니다. 이 구성은 확장 가능한 오브젝트 스토어 주소 지정이 가능하도록 하고, 기본적인 제한 사항을 피하기 위해 권장됩니다.

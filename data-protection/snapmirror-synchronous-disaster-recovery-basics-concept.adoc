...
| 자율적 랜섬웨어 방어 | ONTAP 9.19.1 | ONTAP 9.19.1 RC로 업그레이드하고 SnapMirror 동기 관계의 일부인 볼륨에서 ARP를 활성화하면 관계에서 짧은 동기화되지 않음 상태가 발생할 수 있습니다. 이 상태는 일시적이며 별도의 조치 없이 관계가 자동으로 재동기화됩니다. 

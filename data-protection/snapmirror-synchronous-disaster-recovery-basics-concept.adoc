...
| 자율적 랜섬웨어 방어 | ONTAP 9.19.1 | 업그레이드 후 SnapMirror 동기 또는 SnapMirror 활성 동기화 관계에 있는 볼륨에서 ARP를 활성화하면 해당 관계가 일시적으로 동기화되지 않은 상태가 됩니다. 이 현상은 일시적이며, 별도의 조치 없이 관계가 자동으로 다시 동기화됩니다. 자세한 내용은 link:../anti-ransomware/use-cases-restrictions-concept.html["자율 랜섬웨어 보호 사용 사례 및 고려사항"]을 참조하십시오. 

...
| 애플리케이션이 생성한 스냅샷 복제입니다 | ONTAP 9.7 |  `snapshot create` 작업 시 CLI 또는 ONTAP API를 사용하여 스냅샷에 적절한 레이블이 지정되면 SnapMirror는 애플리케이션을 정지시킨 후 사용자가 생성한 스냅샷이나 외부 스크립트로 생성된 스냅샷을 동기적으로 복제합니다. ONTAP 9.14.1부터는 애플리케이션에서 생성된 스냅샷 복제에 대한 레이블 요구 사항이 더 이상 존재하지 않습니다. 애플리케이션에서 생성된 스냅샷 복제에 대한 자세한 내용은 link:https://kb.netapp.com/Advice_and_Troubleshooting/Data_Protection_and_Security/SnapMirror/How_to_replicate_application_created_snapshots_with_SnapMirror_Synchronous["NetApp 지식 기반: SnapMirror 동기를 사용하여 애플리케이션에서 생성된 스냅샷을 복제하는 방법"^]를 참조하십시오. 


| 자율적 랜섬웨어 방어 | ONTAP 9.19.1 |  
...
| NDMP/NDMP 복구 | ONTAP 9.13.1 | 소스 및 타겟 클러스터 모두 SnapMirror 동기 볼륨의 덤프 또는 SMTape 기반 NDMP 백업을 수행하려면 ONTAP 9.13.1 이상을 실행해야 합니다. 덤프 또는 SMTape 기반 NDMP 복원은 SnapMirror 액티브 동기화 또는 SnapMirror 동기 관계에 있는 타겟 볼륨에 대해 지원되지 않습니다. 
...
| 스냅샷 관리 작업 복제 | ONTAP 9.19.1 | ONTAP 9.19.1부터 스냅샷 관리 작업 `snapshot delete`, `snapshot rename` 및 `snapshot modify`가 보조 클러스터로 복제됩니다. 


| 예약된 스냅샷 복제 | ONTAP 9.19.1 | ONTAP 9.19.1 버전부터는 SnapMirror 정책을 사용하여 예약된 스냅샷 복제를 활성화할 수 있습니다. ONTAP 9.19.1 이전 버전에서는 스냅샷 정책을 사용하여 생성된 예약된 스냅샷은 복제되지 않습니다. 
...
| SVM 재해 복구 | ONTAP 9.6  a| 
* SnapMirror 동기 소스는 SVM 재해 복구 소스로도 사용될 수 있습니다. 예를 들어 SnapMirror 동기를 한쪽으로, SVM 재해 복구를 다른 쪽으로 사용하는 팬아웃 구성이 가능합니다.
* SnapMirror 동기 소스는 SnapMirror 동기가 데이터 보호 소스의 계단식 연결을 지원하지 않으므로 SVM 재해 복구 대상이 될 수 없습니다. 타겟 클러스터에서 SVM 재해 복구 플립 재동기화를 수행하기 전에 동기 관계를 해제해야 합니다.
* SnapMirror 동기 대상은 SVM 재해 복구 소스가 될 수 없습니다. SVM 재해 복구는 DP 볼륨의 복제를 지원하지 않기 때문입니다. 동기 소스의 플립 재동기화가 발생하면 SVM 재해 복구에서 타겟 클러스터의 DP 볼륨이 제외됩니다.




| link:../snaplock/snapshot-lock-concept.html["변조 방지 스냅샷"] | ONTAP 9.19.1 | 스냅샷 생성 시에만 만료 시간을 지정할 수 있습니다. 기존 스냅샷의 만료 시간은 설정하거나 연장할 수 없습니다. 
...
* SnapCenter
* SnapLock 볼륨
* SnapMirror 활성 동기화 또는 SnapMirror 동기 관계의 타겟 볼륨으로 덤프 또는 SMTape 기반 NDMP 복원
* 소스 볼륨의 처리량(QoS Min
...
ONTAP 9.19.1부터 고급 권한 모드에서 SnapMirror 정책 매개변수 `-sync-snapshot-replication-mode`를 사용하여 예약된 스냅샷을 SnapMirror 타겟으로 복제할 수 있습니다. 예약된 스냅샷을 복제하면 SnapMirror 동기화 관계에서 기본 볼륨과 보조 볼륨 간의 대칭성이 제공됩니다.

.필수 구성 요소
* 클러스터 또는 SVM 관리자여야 합니다
* 고급 권한 모드에 있어야 합니다.
* 매개변수 `-sync-snapshot-replication-mode`를 사용하고 옵션 `sched_created`을 포함하는 사용자 정의 SnapMirror 정책을 구성해야 합니다.
+
** 시스템에서 정의한 정책은 수정할 수 없습니다. 새 정책을 생성하고 관계를 다시 설정해야 합니다.
** 사용자 정의 정책 변경 사항이 기존 관계에 적용되려면 재동기화(정지 후 재개)가 필요합니다.




이 매개 변수는 다음 옵션을 허용합니다.

* `none`- 비SnapMirror 동기 스냅샷의 복제를 비활성화합니다.
* `application_created`- 애플리케이션에서 생성한 스냅샷을 복제합니다
* `sched_created` - 예약된 스냅샷을 복제합니다.
* `sm_created` - SnapMirror 비동기(팬아웃 레그)로 생성된 스냅샷을 복제합니다
* 쉼표로 구분된 유형 목록:  `application_created`,  `sched_created` 및  `sm_created`.


해당 매개변수는 관계 상태가 `InSync`인 경우에만 적용됩니다. 나열되지 않은 스냅샷 유형은 운영 볼륨에만 저장됩니다.

ONTAP 9.19.1부터  `snapmirror initialize`은(는) 모든 스냅샷을 보조 클러스터로 복제하는 반면,  `snapmirror resync`은(는) 마지막 공통 스냅샷 이후에 생성된 새 스냅샷만 복제합니다. 재동기화 작업 중에 관계 상태가  `InSync`일 때 기본 전용으로 태그가 지정된 복제되지 않은 스냅샷은 보조 클러스터로 복제되지 않으며, 이러한 스냅샷은 플립 재동기화 작업 후 새 보조 클러스터에 유지됩니다.


NOTE: SnapMirror 동기 볼륨에 스냅샷을 생성하면 사용자 I/O 지연 시간이 증가할 수 있습니다. 스냅샷 생성 주기는 30분보다 길게 설정하는 것이 좋습니다.

.관련 정보
* https://www.netapp.com/pdf.html?item=/media/17174-tr4733.pdf["NetApp 기술 보고서 4733: SnapMirror 동기식 구성 및 모범 사례"^]
* link:https://docs.netapp.com/us-en/ontap-cli//volume-snapshot-create.html["볼륨 스냅샷 생성"^]
* link:https://docs.netapp.com/us-en/ontap-cli/volume-snapshot-delete.html["볼륨 스냅샷 삭제"^]
* link:https://docs.netapp.com/us-en/ontap-cli/volume-snapshot-modify.html["볼륨 스냅샷 수정"^]
* link:https://docs.netapp.com/us-en/ontap-cli/volume-snapshot-rename.html["볼륨 스냅샷 이름 변경"^]

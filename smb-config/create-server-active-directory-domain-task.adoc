...
* ONTAP 9.13.1 버전부터 SMB 서버에 대한 기본 사이트를 구성할 수 있습니다. ONTAP는 LIF에 사이트 멤버십이 없는 경우와 신뢰할 수 있는 도메인 컨트롤러를 찾을 때 신뢰할 수 있는 도메인에 대한 사이트 검색이 실패하는 경우 대체 사이트로 이 기본 사이트를 사용할 수 있습니다.
...
. AD 도메인에 SMB 서버를 생성합니다. `+vserver cifs create -vserver vserver_name -cifs-server smb_server_name -domain FQDN [-ou organizational_unit][-default-site site_name][-netbios-aliases NetBIOS_name, ...][-keytab-uri {(ftp|http)://hostname|IP_address}][-comment text]+`
+
...
ONTAP 9.13.1부터 ONTAP이 신뢰할 수 있는 도메인에 대한 사이트 정보를 검색할 수 없는 경우 SMB 서버 기본 사이트가 대체 사이트로 사용됩니다.

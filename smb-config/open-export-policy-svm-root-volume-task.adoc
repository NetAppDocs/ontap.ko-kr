...
[role="lead"]
SMB 내보내기 정책 규칙을 활성화한 경우 SVM 루트 볼륨의 기본 내보내기 정책에는 모든 클라이언트가 SMB를 통해 자유롭게 액세스할 수 있도록 허용하는 규칙이 포함되어야 합니다. 이러한 규칙이 없으면 모든 SMB 클라이언트는 SVM 및 해당 볼륨에 액세스할 수 없습니다. 내보내기 정책 규칙은 기본적으로 비활성화되어 있습니다.
...
.관련 정보
link:../smb-admin/enable-disable-export-policies-task.html["SMB 액세스를 위한 ONTAP 엑스포트 정책을 설정 또는 해제하는 방법에 대해 알아보십시오"].

...
---
= 4노드 또는 8노드 MetroCluster 구성에서 수동 ONTAP NDU
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
4노드 또는 8노드 MetroCluster 구성의 수동 무중단 업그레이드(NDU)는 업데이트 준비, 하나 또는 두 개의 DR 그룹 각각에서 DR 쌍을 동시에 업데이트하고 업그레이드 후 작업을 수행하는 과정을 포함합니다.

.이 작업에 대해
* 이 작업을 수행하려면 ONTAP 명령줄 인터페이스(CLI)를 사용해야 합니다.
* 이 작업은 다음 구성에 적용됩니다.
+
...
----
+
다음 예에서는 시스템 image2(타겟 ONTAP 이미지)가 node_A_1의 기본 및 현재 버전임을 보여 줍니다.
...
----
+
다음 예에서는 시스템 image2(타겟 ONTAP 이미지)가 node_B_1의 기본 및 현재 버전임을 보여 줍니다.
...
----
+
다음 예에서는 시스템 image2(타겟 ONTAP 이미지)가 node_A_2의 기본 및 현재 버전임을 보여 줍니다.
...
----
+
다음 예에서는 시스템 image2(타겟 ONTAP 이미지)가 node_B_2의 기본 및 현재 버전임을 보여 줍니다.
...
.관련 정보
* link:https://docs.netapp.com/us-en/ontap-cli/storage-failover-giveback.html["스토리지 장애 조치 포기"^]
* link:https://docs.netapp.com/us-en/ontap-cli/storage-failover-modify.html["스토리지 장애 조치 수정"^]
* link:https://docs.netapp.com/us-en/ontap-cli/storage-failover-show-giveback.html["스토리지 장애 조치 쇼-기브백"^]

...
[role="lead"]
자동 무중단 업그레이드(ANDU)를 수행하면 ONTAP가 각 노드에 대상 ONTAP 이미지를 자동으로 설치하고 클러스터가 성공적으로 업그레이드될 수 있는지 확인한 다음 클러스터의 노드 수에 따라 백그라운드에서 xref:concept_upgrade_methods.html[일괄 또는 롤링 업그레이드]을 실행합니다.
...
IMPORTANT: ANDU를 시작하기 전에  `storage failover modify-auto-giveback` 명령 옵션의 설정을 수정해도 업그레이드 프로세스에는 아무런 영향이 없습니다. ANDU 프로세스는 업데이트에 필요한 테이크오버/기브백 중에 이 옵션에 대해 사전 설정된 값을 무시합니다. 예를 들어, ANDU를 시작하기 전에  `-autogiveback`을(를) false로 설정해도 기브백 전의 자동 업그레이드가 중단되지 않습니다. link:https://docs.netapp.com/us-en/ontap-cli/search.html?q=storage+failover+modify-auto-giveback["ONTAP 명령 참조입니다"^]에서  `storage failover modify-auto-giveback`에 대해 자세히 알아보십시오.
...
* 선택적으로, link:../update/pre-stage-system-firmware.html["단일 재부팅 업그레이드를 위한 시스템 펌웨어 사전 준비"].
+
ONTAP 9.17.1부터 ANDU를 수행하기 전에 노드의 부팅 장치에 최신 BIOS 또는 LOADER 펌웨어가 이미 있는 경우, 업그레이드 프로세스는 ONTAP 이미지에 포함된 버전으로 덮어쓰지 않고 최신 펌웨어를 유지합니다. 따라서 ONTAP 소프트웨어 업그레이드와 펌웨어 업데이트를 한 번의 재부팅 주기 내에 완료할 수 있습니다.
...
|===
.. 업그레이드 전 유효성 검사를 실행하려면 *Validate*를 선택하십시오.
+
...
+
자동 업그레이드 사전 검사 목록이 표시되며, 업그레이드 프로세스를 시작하기 전에 해결해야 할 오류 또는 경고도 함께 표시됩니다. 업그레이드를 진행하기 전에 모든 오류를 해결해야 합니다. 경고도 함께 해결하는 것이 좋습니다.
...
====
.다음 단계
펌웨어를 사전 준비한 경우 `system node firmware bootmedia show` 명령을 사용하여 각 노드에서 의도한 펌웨어 버전이 활성화되어 있는지 확인하십시오.

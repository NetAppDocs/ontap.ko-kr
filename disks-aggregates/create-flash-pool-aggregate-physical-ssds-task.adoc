...
summary: HDD RAID 그룹으로 구성된 기존 로컬 계층에서 기능을 활성화한 다음 해당 로컬 계층에 하나 이상의 SSD RAID 그룹을 추가하여 Flash Pool 로컬 계층을 생성합니다. 그러면 해당 로컬 계층에 대해 SSD RAID 그룹 또는 SSD 캐시와 HDD RAID 그룹이라는 두 세트의 RAID 그룹이 생성됩니다. 
---

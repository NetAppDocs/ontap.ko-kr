...
*** 새 버킷에 대한 애그리게이트 및 구성 요소를 지정하지 않으면 새 버킷은 기존 FlexGroup에 추가되지만,  `retention-mode` 설정이 기존 버킷의  `retention-mode` 설정과 일치하고 FlexGroup 유형((`compliance`,  `non-snaplock`,  `enterprise`)이 일치하는 경우에만 가능합니다. 자세한 내용은 xref:../flexgroup/index.html[FlexGroup 볼륨 관리]을 참조하십시오.
+

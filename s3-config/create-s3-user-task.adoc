...
ONTAP 9.19.1부터 원래 액세스 키가 만료되는 경우 S3 리소스에 대한 액세스를 유지하려면 각 S3 사용자에 대해 link:regenerate-access-keys.html#generate-a-second-s3-access-key["두 번째 액세스 키를 생성합니다"]할 수 있습니다.
...
----
+
 `-access-key` 및  `-secret-key` 매개변수에서 사용자를 생성할 때 액세스 키와 비밀 키를 수동으로 정의할 수도 있습니다. 예를 들면 다음과 같습니다.
...
.시스템 관리자
--
. * Storage > Storage VM * 을 선택합니다. 사용자를 추가해야 하는 스토리지 VM을 선택하고 * Settings * 를 선택한 다음 S3에서 image:icon_pencil.gif["편집 아이콘"]를 선택합니다.
. 사용자를 추가하려면 *사용자 > 추가*를 선택합니다.
. 사용자의 이름을 입력합니다.
...
. *저장*을 선택합니다. 사용자가 생성되고 사용자에 대한 액세스 키와 시크릿 키가 생성됩니다.

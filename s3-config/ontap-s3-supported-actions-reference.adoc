...
| PutBucketLifecycleConfiguration을 참조하십시오 | ONTAP 9.13.1 * 만료 작업만 지원됩니다. 
...
ONTAP 9.19.1부터 ONTAP S3는 객체 업로드에 대한 조건부 쓰기 적용 및 객체 삭제에 대한 조건부 삭제 적용을 지원합니다. 조건부 쓰기 및 삭제는 객체의 덮어쓰기 또는 의도하지 않은 삭제를 방지하는 방법을 제공합니다. 조건부 작업은 TLS 연결을 통해 AWS_V4 서명 방법을 사용하여 요청이 이루어진 경우에만 작동합니다. 다음 조건부 헤더 및 객체 작업이 지원됩니다.
...
ONTAP S3는 다음과 같은 추가 URI 쿼리 매개변수를 지원합니다.
`uploadId`
 a| 
ONTAP 9.8
...
| DeleteObjects ONTAP S3는 다음 추가 헤더를 지원합니다. * `x-amz-bypass-governance-retention` | ONTAP 9.11.1 
...
ONTAP S3는 다음과 같은 추가 URI 매개변수를 지원합니다.
...
* ONTAP S3는  `x-amz-content-sha256:
STREAMING-AWS4-HMAC-SHA256-PAYLOAD`을 사용하는 청크 업로드 서명 요청에 대한 지원을 추가합니다.

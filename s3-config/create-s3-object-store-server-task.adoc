...
.이 작업에 대해
객체 저장소 서버가 생성될 때 UID가 0인 루트 사용자가 생성됩니다. 이 루트 사용자에 대한 액세스 키 또는 시크릿 키는 생성되지 않습니다. ONTAP 관리자는 `object-store-server user regenerate-keys` 명령을 실행하여 이 사용자의 액세스 키와 시크릿 키를 설정해야 합니다.

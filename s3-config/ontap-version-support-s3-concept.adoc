...
FC, iSCSI, NFS, NVMe_oF, SMB와 같은 다른 프로토콜과 마찬가지로 S3를 ONTAP에서 사용하려면 라이센스 키를 설치해야 합니다. S3 라이센스 키는 무료 라이센스 키이지만, ONTAP 9.8로 업그레이드하는 시스템에 반드시 설치해야 합니다. S3 라이센스 키는 link:https://mysupport.netapp.com/site/systems/master-license-keys/ontaps3["라이센스 키 페이지"^] NetApp Support 사이트에서 다운로드할 수 있습니다.

...
summary: ONTAP은 Network Data Management Protocol을 통해 테이프 백업 및 복원을 지원합니다. NDMP를 사용하면 스토리지 시스템의 데이터를 테이프에 직접 백업할 수 있으므로 네트워크 대역폭을 효율적으로 사용할 수 있습니다. ONTAP은 테이프 백업을 위해 dump 및 SMTape 엔진을 모두 지원합니다. 
---
= FlexVol 볼륨의 테이프 백업
:allow-uri-read: 
:icons: font

...
summary: 스토리지 가상 머신 범위의 NDMP 모드에서는 NDMP 사용자 인증이 역할 기반 액세스 제어와 통합됩니다. SVM 환경에서 NDMP 사용자는 vsadmin 또는 vsadmin-backup 역할 중 하나를 보유해야 합니다. 클러스터 환경에서 NDMP 사용자는 admin 또는 backup 역할 중 하나를 보유해야 합니다. 
---

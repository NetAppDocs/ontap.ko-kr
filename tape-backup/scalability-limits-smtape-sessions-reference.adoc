...
summary: NDMP 또는 CLI를 통해 SMTape 백업 및 복원 작업을 수행할 때, 시스템 메모리 용량이 다른 스토리지 시스템에서 동시에 수행할 수 있는 최대 SMTape 백업 및 복원 세션 수를 숙지해야 합니다. 이 최대 수는 스토리지 시스템의 시스템 메모리에 따라 달라집니다. 
---
